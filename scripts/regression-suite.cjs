...
;(async () => {
  ...
  const api = await page.evaluate(() => {
    const get = (k) => { try { return JSON.parse(localStorage.getItem('kailash.api.v1.' + k) || '[]') } catch { return [] } }
  ...
  const legacy = await page.evaluate(() => {
    ...
    const er   = safe('kailash-erstore')
    const ins  = safe('kailash-insurancestore')
    const ot   = safe('kailash-otstore')
    const nrc  = safe('kailash-narcoticsstore')
    const dm   = safe('kailash-drugmasterstore')
    const inp  = safe('kailash-ipd')
    return {
  ...
  const persistedKeys = await page.evaluate(() => {
    ...
    for (let i = 0; i < window.localStorage.length; i++) {
      ...
      if (k && k.startsWith('kailash')) out.push(k)
    }
    return out.sort()
  })
  report.probes.push({ name: 'persistence', keys: persistedKeys })
  assert('Persisted localStorage keys >= 20',  persistedKeys.length >= 20)
  assert('Mock-API patients persisted',         persistedKeys.includes('kailash.api.v1.patients'))
  assert('Audit table persisted',                persistedKeys.includes('kailash.api.v1.audit_entries'))
  assert('Auth persisted (sticky demo)',         persistedKeys.includes('kailash-authstore'))
