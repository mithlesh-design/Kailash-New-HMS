...
;(async () => {
  ...
  const lsTable = (table) => ls(`kailash.api.v1.${table}`)
  ...
  // ═══════════════════════════════════════════════════════════════
  {
    ...
    const ui = Array.isArray(persisted) && persisted.length >= 20
              ? `${persisted.length} audit rows persisted in kailash.api.v1.audit_entries`
  ...
  // ═══════════════════════════════════════════════════════════════
  {
    ...
    const narc = await page.evaluate(() => {
      try { return JSON.parse(localStorage.getItem('kailash-narcoticsstore') || '{}')?.state?.log || [] }
      catch { return [] }
    })
