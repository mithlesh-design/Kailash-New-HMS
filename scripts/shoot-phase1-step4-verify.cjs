...
;(async () => {
  ...
  const apiCheck = await page.evaluate(() => {
    try {
      const ps = JSON.parse(localStorage.getItem('kailash.api.v1.patients') || '[]')
      const stays = JSON.parse(localStorage.getItem('kailash.api.v1.ipd_stays') || '[]')
      const bills = JSON.parse(localStorage.getItem('kailash.api.v1.bills') || '[]')
      const rxs   = JSON.parse(localStorage.getItem('kailash.api.v1.prescriptions') || '[]')
      const labs  = JSON.parse(localStorage.getItem('kailash.api.v1.lab_results') || '[]')
      const dis   = JSON.parse(localStorage.getItem('kailash.api.v1.discharges') || '[]')
      const aud   = JSON.parse(localStorage.getItem('kailash.api.v1.audit_entries') || '[]')
      return {
  ...
  const persist = await page.evaluate(() => {
    ...
    for (let i = 0; i < window.localStorage.length; i++) {
      ...
      if (k && k.startsWith('kailash')) all.push(k)
    }
    return all.sort()
  })
  ...
  assert('Auth persisted',                       persist.includes('kailash-authstore'))
  assert('Mock-API audit_entries persisted',     persist.includes('kailash.api.v1.audit_entries'))
  assert('Mock-API patients persisted',          persist.includes('kailash.api.v1.patients'))
