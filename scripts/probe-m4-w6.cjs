...
;(async () => {
  ...
  for (let i = 0; i < 60; i++) {
    const ready = await p.evaluate(() =>
      ...
      Object.keys(localStorage).some((k) => k.startsWith('kailash.legacy-seed.anil-')))
    if (ready) break
    await sleep(500)
  }
  await sleep(2500)
  ...
  await p.evaluate(() => {
    try {
      const raw = localStorage.getItem('kailash-authstore')
      ...
      localStorage.setItem('kailash-authstore', JSON.stringify(o))
    } catch {}
  })
