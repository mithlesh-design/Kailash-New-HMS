...
;(async () => {
  ...
  const apiCheck = await page.evaluate(async () => {
    ...
    try {
      const ks = Object.keys(window.localStorage).filter(k => k.startsWith('kailash.api.v1.'))
      out.boot = ks.includes('kailash.api.v1.__bootstrap__')
      out.patientCount = JSON.parse(window.localStorage.getItem('kailash.api.v1.patients') || '[]').length
      const ps = JSON.parse(window.localStorage.getItem('kailash.api.v1.patients') || '[]')
      out.kiran = ps.find(p => p.id === 'PT-20394') ? true : false
      const stays = JSON.parse(window.localStorage.getItem('kailash.api.v1.ipd_stays') || '[]')
      out.kiranIpd = stays.find(s => s.patientId === 'PT-20394') ? true : false
      out.auditCount = JSON.parse(window.localStorage.getItem('kailash.api.v1.audit_entries') || '[]').length
      out.billCount = JSON.parse(window.localStorage.getItem('kailash.api.v1.bills') || '[]').length
      out.prescriptionCount = JSON.parse(window.localStorage.getItem('kailash.api.v1.prescriptions') || '[]').length
