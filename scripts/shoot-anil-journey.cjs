...
;(async () => {
  ...
  const api = await page.evaluate(() => {
    const get = (key) => { try { return JSON.parse(localStorage.getItem('kailash.api.v1.' + key) || '[]') } catch { return [] } }
    ...
    return {
      ...
      legacy_seed_marker: !!localStorage.getItem('kailash.legacy-seed.anil-v2'),
    }
  })
  ...
  const legacy = await page.evaluate(() => {
    ...
    const er  = safeGet('kailash-erstore')?.state || {}
    const ins = safeGet('kailash-insurancestore')?.state || {}
    const ot  = safeGet('kailash-otstore')?.state || {}
    const nrc = safeGet('kailash-narcoticsstore')?.state || {}
    const drg = safeGet('kailash-drugmasterstore')?.state || {}
    const adm = safeGet('kailash-admissionstore')?.state || {}
