...
;(async () => {
  ...
  const before = await page.evaluate(() => Object.keys(window.localStorage).filter(k => k.startsWith('kailash')).length)
  ...
  const after = await page.evaluate(() => {
    return Object.keys(window.localStorage).filter(k => k.startsWith('kailash')).sort()
  })
