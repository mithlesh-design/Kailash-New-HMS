...
;(async () => {
  ...
  const persistedHasEvent = await page.evaluate(() => (localStorage.getItem('kailash-ipd') || '').includes('Order actioned'))
  console.log('[diag] persisted localStorage has "Order actioned":', persistedHasEvent)
  const pv = await page.evaluate(() => { try { return JSON.parse(localStorage.getItem('kailash-ipd')).version } catch { return 'none' } })
  ...
  const persistedAfter = await page.evaluate(() => (localStorage.getItem('kailash-ipd') || '').includes('Order actioned'))
