...
KAREXPERT_AREAS = [
    ("Module breadth",
     ...
     "Parity. Kailash already covers every area for the demo, persistent in-browser.",
     "n/a"),
    ("UI polish",
     "Mature SaaS look-and-feel with consistent components across modules.",
     "Behind in M0. Closing fast with M2's compact design system + command palette. Visible difference: KareXpert lists; Kailash lists + Cmd-K intent navigation.",
...
def build(out: Path) -> None:
    ...
    p(doc,
      ...
      "KareXpert-vs-Kailash teardown, then a prioritised slate of NEW "
    ...
    p(doc,
      ...
      "Kailash is already at parity vs. where the slate leaps.")
    table(doc, ["Area", "KareXpert today", "Kailash position", "Slate items"],
          [list(r) for r in KAREXPERT_AREAS],
          col_widths_cm=[3.0, 4.5, 4.5, 3.0])

    h1(doc, "3. Where Kailash Already Leads (post-M0/M1/M2/M3)")
