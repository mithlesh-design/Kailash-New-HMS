...
interface InpatientState {
  ...
  signConsent: (patientId: string) => void
...
export const useInpatientStore = create<InpatientState>()(
  persist(
    (set, get) => ({
      ...
      signConsent: (id) => set(s => patch(s, id, ip => ip.surgery ? ({
        ...ip, surgery: { ...ip.surgery, consentSigned: true },
        events: append(ip, { type: 'surgery_status', actor: ip.name, title: 'Consent signed', severity: 'success', patientText: 'You gave consent for your procedure.' }),
      }) : ip)),
      ...
      completeDischarge: (id) => set(s => patch(s, id, ip => ip.discharge ? ({
    ...
    {
      name: 'kailash-ipd', version: 5, storage: createJSONStorage(() => localStorage), skipHydration: true,
