...
function makeEvents(dischargedOn: string, followUpDate?: string): PostDischargeEvent[] {
  ...
  if (followUpDate) {
    ...
    events.push({
      ...
      message: 'Reminder: Your follow-up appointment is tomorrow at Kailash Hospital.',
    })
    events.push({
      id: 'EV-4', type: 'doctor_availability', label: 'Follow-up Day Confirmation',
      scheduledDate: followUpDate, channel: 'sms', status: 'scheduled',
      message: 'Your doctor is available today for your follow-up. Kailash Hospital OPD timings: 9am–1pm.',
    })
  }
  return events
}
