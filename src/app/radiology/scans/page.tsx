    ...
    <div className={cn("flex items-center gap-2 mt-2 text-xs font-semibold", overdue ? "text-red-600" : "text-slate-500")}>
      ...
      <div className="flex-1">
        ...
        <div className="h-1.5 bg-slate-200 rounded-full overflow-hidden">
          <div
            className={cn("h-full rounded-full transition-all", overdue ? "bg-red-500" : pct > 80 ? "bg-orange-400" : "bg-indigo-500")}
            style={{ width: `${pct}%` }}
          />
        </div>
      </div>
        ...
        {(['All', 'Urgent', 'Overdue', ...STATUS_ORDER] as const).map(f => (
          <button
            ...
            className={cn("text-sm font-bold px-3 py-1.5 rounded-lg transition-all cursor-pointer relative",
              filter === f ? 'bg-indigo-600 text-white shadow-sm' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50'
            )}
          >
            {f}
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        ...
                        {STATUS_NEXT[scan.status] && (
                          <button
                            ...
                            className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-indigo-50 hover:bg-indigo-100 text-indigo-700 text-xs font-bold transition-colors cursor-pointer border border-indigo-200"
                          >
