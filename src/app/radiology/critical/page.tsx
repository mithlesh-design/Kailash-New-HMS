                      ...
                      {!s.acknowledgedAt && (
                        <div className="mt-2 max-w-xs">
                          ...
                          <div className="h-1.5 rounded-full bg-slate-100 overflow-hidden">
                            <div className={cn("h-full rounded-full", breached ? "bg-red-500" : pct > 70 ? "bg-amber-500" : "bg-[#2563EB]")} style={{ width: `${pct}%` }} />
                          </div>
                        </div>
                      )}
                        ...
                        {callbackFor === s.id ? (
                          <div className="flex items-center gap-1.5">
                            <input autoFocus value={recipient} onChange={e => setRecipient(e.target.value)}
                              placeholder={s.doctorName} className="h-8 w-36 px-2 rounded-lg text-[12px] border border-[#EAECF2] focus:outline-none focus:border-[#2563EB]" />
                            <button onClick={() => onCallback(s)} className="h-8 px-3 rounded-lg text-[12px] font-semibold bg-[#1E3A8A] text-white hover:bg-[#172E6E] cursor-pointer">Log</button>
                          </div>
                        ) : (
