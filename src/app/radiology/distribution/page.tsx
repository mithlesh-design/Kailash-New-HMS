  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2">
        <span className="h-9 w-9 rounded-xl flex items-center justify-center bg-[#1E3A8A]/[0.08] text-[#1E3A8A]"><Send className="h-5 w-5" /></span>
        <div>
            ...
            <div key={s.id} className="rounded-2xl border border-[#EAECF2] bg-white shadow-[var(--shadow-card)] p-5">
              <div className="flex items-start justify-between gap-3 flex-wrap">
                ...
                <button onClick={() => setOpenSummary(openSummary === s.id ? null : s.id)}
                  className="inline-flex items-center gap-1.5 h-8 px-3 rounded-full text-[12px] font-semibold bg-[#F5F8FF] text-[#1E3A8A] border border-[#2563EB]/15 hover:border-[#2563EB]/30 cursor-pointer flex-shrink-0">
                  <Sparkles className="h-3.5 w-3.5" /> {openSummary === s.id ? "Hide" : "Patient summary"}
                </button>
              </div>

              {openSummary === s.id && (
                <div className="mt-3 rounded-xl bg-[#F5F8FF] border border-[#2563EB]/15 p-3.5">
                  <p className="text-[10px] font-bold uppercase tracking-wide text-[#1E3A8A] mb-1 flex items-center gap-1"><Sparkles className="h-3 w-3" /> AI plain-language summary</p>
                  ...
                  return (
                    <button key={ch.key} onClick={() => send(s, ch)}
                      className={cn("inline-flex items-center gap-1.5 h-8 px-3 rounded-lg text-[12px] font-semibold border transition-colors cursor-pointer",
                        sent ? "bg-emerald-50 text-emerald-700 border-emerald-200" : "bg-white text-[#344054] border-[#EAECF2] hover:border-[#2563EB] hover:text-[#1E3A8A]")}>
