      ...
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          <div className="flex items-center gap-2">
            <span className="h-9 w-9 rounded-xl flex items-center justify-center bg-[#1E3A8A]/[0.08] text-[#1E3A8A]"><BarChart3 className="h-5 w-5" /></span>
        ...
        <div className="inline-flex gap-1 p-1 rounded-full bg-[#F8FAFC] border border-[#EAECF2]">
          {PERIODS.map(p => (
            <button key={p} onClick={() => setPeriod(p)}
              className={cn("px-3 py-1.5 rounded-full text-[12.5px] font-semibold transition cursor-pointer", period === p ? "bg-[#1E3A8A] text-white" : "text-slate-500 hover:text-slate-800")}>
              {p}
            </button>
          ))}
        </div>
          ...
          <ClientOnly fallback={<div className="h-[240px]" />}>
            <ResponsiveContainer width="100%" height={240}>
              <LineChart data={m.tatTrend} margin={{ top: 6, right: 12, left: -14, bottom: 0 }}>
                ...
                <Line type="monotone" dataKey="tat" name="Actual TAT" stroke="#2563EB" strokeWidth={2} dot={false} />
                ...
                return (
                  <div key={r.name}>
                    ...
                    <div className="h-2 rounded-full bg-slate-100 overflow-hidden">
                      <div className="h-full bg-[#2563EB] rounded-full" style={{ width: `${(r.count / max) * 100}%` }} />
                    </div>
                  </div>
                )
              })}
            ...
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={m.byModality} margin={{ top: 6, right: 8, left: -20, bottom: 0 }}>
                ...
                <Bar dataKey="util" radius={[4, 4, 0, 0]}>
                  {m.byModality.map((x, i) => <Cell key={i} fill={x.util > 85 ? "#1E3A8A" : "#2563EB"} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </ClientOnly>
