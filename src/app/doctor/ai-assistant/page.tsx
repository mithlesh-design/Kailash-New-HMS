      ...
      <aside className="hidden md:flex w-60 flex-shrink-0 flex-col rounded-2xl bg-white shadow-[0_1px_4px_rgba(15,23,42,0.06)] overflow-hidden">
        <div className="p-3 border-b border-slate-100">
          <button onClick={() => newThread()}
            className="w-full h-10 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-semibold text-[13.5px] flex items-center justify-center gap-2 hover:opacity-95 transition">
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1 min-h-0">
          {!mounted ? null : threads.length === 0 ? (
            ...
            <button key={t.id} onClick={() => selectThread(t.id)}
              className={cn("group w-full text-left px-3 py-2.5 rounded-xl flex items-start gap-2.5 transition", t.id === activeId ? "bg-blue-50" : "hover:bg-slate-50")}>
              <MessageSquare className={cn("h-4 w-4 mt-0.5 flex-shrink-0", t.id === activeId ? "text-blue-600" : "text-slate-400")} />
              <span className="flex-1 min-w-0">
                <span className={cn("block text-[13px] font-semibold truncate", t.id === activeId ? "text-blue-900" : "text-slate-700")}>{t.title}</span>
      ...
      <section className="flex-1 flex flex-col min-w-0 rounded-2xl bg-white shadow-[0_1px_4px_rgba(15,23,42,0.06)] overflow-hidden">
        ...
        <div className="px-5 py-3.5 border-b border-slate-100 flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-2.5">
            <span className="h-9 w-9 rounded-xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center shadow-sm">
            ...
            <div className="flex items-end gap-2">
              <div className="flex-1 relative">
                <textarea
                  ...
                  className="w-full resize-none max-h-32 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-[14px] text-slate-800 placeholder:text-slate-400 outline-none focus:border-blue-300 focus:ring-2 focus:ring-blue-100"
                />
              </div>
              <button onClick={() => send(input)} disabled={!input.trim() || thinking} aria-label="Send"
                className="h-11 w-11 flex-shrink-0 rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-600 text-white flex items-center justify-center disabled:opacity-40 hover:opacity-95 transition">
...
function EmptyHero({ focusName, onPick }: { focusName?: string; onPick: (q: string) => void }) {
  return (
    <div className="h-full flex flex-col items-center justify-center text-center max-w-lg mx-auto px-4">
      <span className="h-14 w-14 rounded-2xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center shadow-lg mb-4">
      ...
      {focusName && <p className="text-[12px] font-semibold text-blue-600 mt-3 bg-blue-50 px-3 py-1 rounded-full">Pinned: {focusName}</p>}
      <div className="mt-6 flex flex-wrap gap-2 justify-center">
        {QUICK_PROMPTS.map(p => (
          <button key={p} onClick={() => onPick(p)}
            className="text-[12.5px] font-medium text-slate-600 bg-white border border-slate-200 rounded-full px-3.5 py-2 hover:border-blue-300 hover:text-blue-700 hover:bg-blue-50 transition">
            {p}
          </button>
        ))}
      </div>
...
function Bubble({ role, text, draft }: { role: 'user' | 'ai'; text: string; draft?: AssistantDraft }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}
      ...
      {!isUser && (
        <span className="h-8 w-8 rounded-xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center flex-shrink-0 mt-0.5">
      ...
      <div className={cn("max-w-[80%] space-y-2", isUser && "flex flex-col items-end")}>
        <div className={cn("rounded-2xl px-4 py-2.5 text-[13.5px] leading-relaxed",
          isUser ? "bg-blue-600 text-white rounded-br-md" : "bg-white border border-slate-200 text-slate-700 rounded-bl-md shadow-sm")}>
          <Rich text={text} />
        </div>
...
function DraftCard({ draft }: { draft: AssistantDraft }) {
  ...
  return (
    <div className="w-full rounded-2xl border border-blue-200 bg-blue-50/40 overflow-hidden shadow-sm">
      <div className="flex items-center justify-between px-3.5 py-2 bg-white/70 border-b border-blue-100">
        <span className="flex items-center gap-2 text-[12px] font-bold text-blue-800">
          <Icon className="h-3.5 w-3.5" /> {draft.title}
        </span>
        <span className="text-[10px] font-bold uppercase tracking-wider text-blue-400">AI draft</span>
      </div>
      ...
      <div className="flex items-center gap-2 px-3.5 py-2.5 border-t border-blue-100 bg-white/50">
        ...
        {canExecute && (
          <button onClick={run} disabled={done}
            className={cn("flex items-center gap-1.5 text-[12px] font-semibold text-white rounded-lg px-3 py-1.5 transition disabled:opacity-60", confirming ? "bg-amber-600 hover:bg-amber-700" : "bg-blue-600 hover:bg-blue-700")}>
...
function ThinkingBubble() {
  return (
    <div className="flex gap-2.5 justify-start">
      <span className="h-8 w-8 rounded-xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center flex-shrink-0">
      ...
      <div className="rounded-2xl rounded-bl-md bg-white border border-slate-200 px-4 py-3 shadow-sm flex items-center gap-1">
        {[0, 1, 2].map(i => <span key={i} className="h-1.5 w-1.5 rounded-full bg-blue-400 animate-bounce" style={{ animationDelay: `${i * 0.15}s` }} />)}
      </div>
    </div>
  )
}
