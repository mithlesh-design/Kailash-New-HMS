...
const field = "w-full h-10 rounded-xl border border-slate-200 px-3 text-sm text-slate-900 outline-none focus:border-blue-300 focus:ring-2 focus:ring-blue-100"
...
function Toggle({ on, onClick, label }: { on: boolean; onClick: () => void; label: string }) {
  return (
    <button onClick={onClick} className="flex items-center justify-between w-full">
      ...
      <span className={cn("h-6 w-11 rounded-full transition-colors relative flex-shrink-0", on ? "bg-blue-600" : "bg-slate-200")}>
...
export default function DoctorSettings() {
  ...
  return (
    <div className="max-w-3xl mx-auto pb-10 space-y-4">
      ...
      {/* Availability */}
      <div className={CARD}>
        <h3 className="text-[15px] font-bold text-slate-900 mb-4 flex items-center gap-2"><Stethoscope className="h-4.5 w-4.5 text-blue-600" /> Availability</h3>
      ...
      <button onClick={() => toast.success('Settings saved')} className="w-full h-12 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-[14px] flex items-center justify-center gap-2 transition">
