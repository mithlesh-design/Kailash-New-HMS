...
export default function DoctorDashboard() {
  ...
  const handleSendAdmission = () => {
    ...
    markAdmissionSent()
    toast.success("Admission request sent to Bed Manager")
  }
              ...
              <div className="space-y-3 mt-3">
                {admissionOrder?.sent ? (
                  <div className="flex items-center gap-3 p-4 rounded-xl" style={{ background: '#F0FDF4' }}>
                    ...
                    <div>
                      <p className="text-sm font-bold text-green-900">Admission Request Sent</p>
                  ...
                  <>
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <select className={selectStyle} style={selectInlineStyle}>
                        <option>Any bed</option><option>Near nurse station</option><option>Ground floor</option><option>Isolation</option>
                      </select>
                    </div>
                    <Input placeholder="Reason for admission..." value={admReason} onChange={e => setAdmReason(e.target.value)} className="bg-[#F8FAFC]" />
                    <Button variant="danger" className="w-full gap-2" onClick={() => {
                      if (!admReason) { toast.error("Please enter reason for admission"); return }
                      setAdmissionOrder({ admissionType: admType, reason: admReason, bedTypePreference: admType })
                      handleSendAdmission()
                    }}>
                      <UserPlus className="h-4 w-4" /> Request Admission
                    </Button>
                  </>
                )}
              </div>
