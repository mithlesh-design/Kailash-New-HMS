...
const SLOT_STYLE: Record<string, { badge: string; label: string; row: string }> = {
  ...
  'in-progress':{ badge: 'bg-blue-600 text-white',  label: 'In progress', row: 'bg-blue-50/50 border-blue-200' },
  ...
  return (
    <div className="max-w-3xl mx-auto pb-8 space-y-5">
      ...
      <div className="rounded-2xl bg-white shadow-[0_1px_4px_rgba(15,23,42,0.06)] p-4 flex flex-wrap items-center gap-2.5">
        ...
        <span className={cn("text-[11.5px] font-semibold px-2 py-0.5 rounded-full flex items-center gap-1", profile.availableForOnline ? "bg-violet-50 text-violet-700" : "bg-slate-100 text-slate-400")}><Video className="h-3 w-3" /> Online {profile.availableForOnline ? 'on' : 'off'}</span>
          ...
          return (
            <motion.div key={p.id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
              ...
              {done ? (
                ...
                <button onClick={() => openConsult(p.id)} className="w-28 h-9 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-[12.5px] font-bold flex items-center justify-center gap-1.5 active:scale-95 transition">
          ...
          <div className="space-y-2.5">
            {upcomingAppts.map(a => (
              <div key={a.id} className="flex items-center gap-4 rounded-2xl border border-slate-200 bg-white p-4">
                <span className={cn("h-9 w-9 rounded-xl flex items-center justify-center flex-shrink-0", a.mode === 'online' ? 'bg-violet-50 text-violet-600' : 'bg-teal-50 text-teal-600')}>
