  ...
  return (
    <div className="pb-6">
      <div className="mb-4">
        ...
        <p className="text-[13px] text-slate-500 mt-0.5">Live across Kailash branches — find a bed when your ward is full</p>
      </div>
        ...
        {branches.map(b => {
          ...
          return (
            <button key={b.id} onClick={() => setBranchId(b.id)}
              className={cn("flex items-center gap-2.5 rounded-2xl border px-4 py-2.5 text-left transition", active ? "border-blue-300 bg-blue-50/60 ring-1 ring-blue-200" : "border-slate-200 bg-white hover:bg-slate-50")}>
              <span className={cn("h-9 w-9 rounded-xl flex items-center justify-center flex-shrink-0", active ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-500")}><Building2 className="h-4.5 w-4.5" /></span>
              <div>
                <p className="text-[13px] font-bold text-slate-900 leading-tight">{b.location}{b.id === CURRENT_BRANCH.id && <span className="ml-1.5 text-[10px] font-bold text-blue-600">· This branch</span>}</p>
              ...
              <tr>
                ...
                {branches.map(b => (
                  <th key={b.id} className="text-center text-[11px] font-bold uppercase tracking-wider text-slate-400 pb-2 px-2">
                    ...
                    {b.id === CURRENT_BRANCH.id && <span className="text-blue-600"> here</span>}
                  </th>
                ))}
              </tr>
            </thead>
