...
export default function DoctorInbox() {
  ...
  return (
    <div className="pb-2 h-full flex flex-col min-h-0">
      ...
      <ClientOnly fallback={<div className="flex-1 flex items-center justify-center"><div className="h-7 w-7 rounded-full border-4 border-blue-200 border-t-blue-600 animate-spin" role="status" aria-label="Loading inbox" /></div>}>
        {tab === 'messages' ? (
          ...
          <div className="flex-1 min-h-0 overflow-y-auto space-y-3">
            {unreadAlerts > 0 && (
              <div className="flex justify-end">
                <button onClick={() => markAllRead('doctor')} className="text-[12.5px] font-semibold text-blue-600 hover:text-blue-700">Mark all read</button>
              </div>
            )}
            {alerts.map(n => (
              <div key={n.id} className={cn("p-4 rounded-xl border bg-white", n.priority === 'critical' ? "border-l-4 border-l-red-500" : n.priority === 'high' ? "border-l-4 border-l-orange-400" : "border-l-4 border-l-slate-200", !n.read && "ring-1 ring-blue-200 bg-blue-50/30")}>
      ...
      {results.map(r => {
        ...
        return (
          <div key={r.key} className={cn("p-4 rounded-xl bg-white flex items-start justify-between gap-3 border", r.critical ? "border-l-4 border-l-red-500 ring-1 ring-red-100" : "border-slate-200")}>
            ...
            <button onClick={() => onAck(r)} className="flex-shrink-0 h-9 px-3 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-[12px] font-bold flex items-center gap-1.5"><CheckCircle className="h-3.5 w-3.5" /> Sign off</button>
          </div>
        )
      })}
    </div>
