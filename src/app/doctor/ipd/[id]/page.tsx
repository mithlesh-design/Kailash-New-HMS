...
export default function InpatientChart() {
  ...
  if (!mounted) return (
    <div className="h-full flex items-center justify-center">
      <div className="h-8 w-8 rounded-full border-4 border-blue-200 border-t-blue-600 animate-spin" role="status" aria-label="Loading chart" />
    </div>
  )

  if (!ip) return (
    <div className="p-8 text-slate-500">
      Patient not found. <button onClick={() => router.push('/doctor/ipd')} className="text-blue-600 font-semibold">Back to IPD</button>
    </div>
  )
    ...
    <div className="flex flex-col lg:flex-row gap-4 h-full min-h-0">
      ...
      <section className="flex-1 min-w-0 flex flex-col rounded-2xl bg-white shadow-[0_1px_4px_rgba(15,23,42,0.06)] overflow-hidden">
        <div className="px-3 py-2.5 border-b border-slate-100 flex items-center gap-1 overflow-x-auto">
          {TABS.map(t => (
            <button key={t} onClick={() => setTab(t)} className={cn("px-3 py-1.5 rounded-lg text-[13px] font-semibold whitespace-nowrap transition", t === tab ? "bg-blue-50 text-blue-700" : "text-slate-500 hover:text-slate-700")}>{t}</button>
          ))}
        </div>
