      ...
      <div className="flex items-center gap-1 p-1 rounded-xl bg-slate-100 w-fit mb-4">
        {TABS.map(({ key, label }) => (
          <button key={key} onClick={() => setTab(key)}
            ...
            {label} <span className={cn("text-[11px] font-bold px-1.5 rounded-full", tab === key ? "bg-blue-100 text-blue-700" : "bg-slate-200 text-slate-500")}>{counts[key]}</span>
          </button>
        ))}
      </div>
                  ...
                  <tr key={r.id} onClick={() => setSelectedId(r.id)}
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        <span className={cn("h-9 w-9 rounded-xl text-white flex items-center justify-center font-bold text-[12.5px] flex-shrink-0", r.mode === 'online' ? "bg-gradient-to-br from-violet-500 to-fuchsia-500" : "bg-gradient-to-br from-blue-500 to-cyan-500")}>{initials(r.name)}</span>
                    ...
                    <td className="px-4 py-3">
                      <span className={cn("inline-flex items-center gap-1 text-[11px] font-bold px-2 py-0.5 rounded-full", r.mode === 'online' ? "bg-violet-50 text-violet-700" : "bg-teal-50 text-teal-700")}>
          ...
          <>
            ...
            <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'tween', duration: 0.25, ease: [0.16, 1, 0.3, 1] }}
              ...
              <div className="sticky top-0 bg-white border-b border-slate-100 px-5 py-4 flex items-start justify-between gap-3 z-10">
                <div className="flex items-center gap-3">
                  <span className={cn("h-12 w-12 rounded-2xl text-white flex items-center justify-center font-bold text-[16px]", selected.mode === 'online' ? "bg-gradient-to-br from-violet-500 to-fuchsia-500" : "bg-gradient-to-br from-blue-500 to-cyan-500")}>{initials(selected.name)}</span>
                  <div>
                    <p className="text-[17px] font-bold text-slate-900 leading-tight">{selected.name}</p>
                    <p className="text-[12.5px] text-slate-500">{selected.id} · {selected.age}y · {selected.gender} · <span className={cn("font-semibold", selected.mode === 'online' ? "text-violet-600" : "text-teal-600")}>{selected.mode === 'online' ? 'Online' : 'In-person'}</span></p>
                  </div>
                </div>
              ...
              <div className="sticky bottom-0 bg-white border-t border-slate-100 p-4">
                <button onClick={() => openConsult(selected)} className="w-full h-11 rounded-xl bg-blue-600 text-white font-bold text-[13.5px] flex items-center justify-center gap-2 hover:bg-blue-700 transition">
