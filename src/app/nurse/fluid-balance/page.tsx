  ...
  return (
    <Card className="p-5">
      ...
      <div className="grid grid-cols-3 gap-2 mt-3 mb-3">
        {[
          { label: "Intake", value: `${bal.intake} mL`, cls: "text-blue-600" },
          ...
          {ivs.map(l => {
            ...
            return (
              <div key={l.id} className="flex items-center gap-3 rounded-xl border border-slate-100 bg-white p-2.5">
                <Droplets className={`h-4 w-4 flex-shrink-0 ${l.status === "Running" ? "text-blue-500" : "text-slate-300"}`} />
          ...
          <div className="flex rounded-lg overflow-hidden border border-slate-200">
            {(["intake", "output"] as IoKind[]).map(k => (
              <button key={k} onClick={() => { setKind(k); setType(k === "intake" ? INTAKE_TYPES[0] : OUTPUT_TYPES[0]) }}
                className={`px-3 py-1.5 text-xs font-bold cursor-pointer ${kind === k ? (k === "intake" ? "bg-blue-600 text-white" : "bg-amber-500 text-white") : "bg-white text-slate-500"}`}>
