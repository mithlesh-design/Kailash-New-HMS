    ...
    <div className="space-y-6">
      ...
      {pendingIncoming.length > 0 && (
        <Card className="p-5 border border-blue-200 bg-blue-50/60">
          <div className="flex items-center gap-2 mb-3"><Inbox className="h-4 w-4 text-blue-600" /><h2 className="text-sm font-bold text-blue-900">Incoming handover — start of shift</h2></div>
          {pendingIncoming.map(h => (
            <div key={h.id} className="rounded-xl bg-white border border-blue-100 p-4">
              <div className="flex items-center justify-between gap-3 flex-wrap mb-2">
                <p className="text-sm font-bold text-slate-800 flex items-center gap-2"><ArrowRightLeft className="h-3.5 w-3.5 text-blue-500" /> {h.fromNurse} ({h.fromShift}) → you ({h.toShift})</p>
