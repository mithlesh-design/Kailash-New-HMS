...
const RX_STATUS: Record<PrepStatus, { label: string; tint: string }> = {
  ...
  preparing: { label: "Being prepared", tint: "bg-blue-50 text-blue-700" },
          ...
          return (
            <div key={m.name} className="rounded-3xl bg-white shadow-[0_1px_4px_rgba(15,23,42,0.06),0_8px_28px_rgba(15,23,42,0.05)] p-5">
              ...
              <div className="mt-4 flex flex-wrap items-center gap-2">
                ...
                {m.times.map(t => <span key={t} className="text-[12.5px] font-semibold px-2.5 py-1 rounded-lg bg-blue-50 text-blue-700">{t}</span>)}
              </div>
              ...
              <div className="mt-3 flex items-center justify-between">
                ...
                <button
                  ...
                  className={cn(
                    ...
                    isOrdered ? "bg-green-50 text-green-700 cursor-default" : "bg-blue-600 text-white",
                  )}
                >
      ...
      <div className="rounded-3xl bg-white shadow-[0_1px_4px_rgba(15,23,42,0.06),0_8px_28px_rgba(15,23,42,0.05)] p-5">
        <div className="flex items-center justify-between mb-1">
          <h3 className="text-[15px] font-bold text-slate-900 flex items-center gap-2">
            <span className="h-10 w-10 rounded-2xl bg-blue-50 flex items-center justify-center flex-shrink-0"><Truck className="h-5 w-5 text-blue-600" /></span>
            Home delivery
          </h3>
          <span className="text-[11px] font-bold px-2.5 py-1 rounded-full bg-blue-50 text-blue-700">ETA today, 6:30 PM</span>
        </div>
          ...
          {DELIVERY_STAGES.map((s, i) => (
            <div key={s.label} className="flex items-center flex-1 last:flex-none">
              <div className="flex flex-col items-center gap-1">
                <div className={cn("h-8 w-8 rounded-full flex items-center justify-center", i <= DELIVERY_CURRENT ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-400")}>
                  {i < DELIVERY_CURRENT ? <CheckCircle className="h-4.5 w-4.5" /> : <s.Icon className="h-4 w-4" />}
                </div>
                <span className={cn("text-[11px] font-semibold text-center", i <= DELIVERY_CURRENT ? "text-blue-600" : "text-slate-400")}>{s.label}</span>
              </div>
              {i < DELIVERY_STAGES.length - 1 && <div className={cn("flex-1 h-0.5 mx-1 -mt-4 rounded", i < DELIVERY_CURRENT ? "bg-blue-600" : "bg-slate-200")} />}
            </div>
          ))}
        </div>
      ...
      <div className="rounded-3xl bg-white shadow-[0_1px_4px_rgba(15,23,42,0.06),0_8px_28px_rgba(15,23,42,0.05)] p-5">
        ...
        <div className="rounded-2xl bg-slate-50 p-4 space-y-2">
          ...
          <div className="border-t border-slate-200 pt-2 flex items-center justify-between font-bold text-[14px]">
            ...
            <span className="text-blue-600">₹{billTotal}</span>
          </div>
        </div>
      </div>
    </div>
