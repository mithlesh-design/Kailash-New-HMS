      ...
      {isUpcoming && !cancelled && (
        <div className="mt-4 flex flex-wrap items-center gap-2">
          {c.mode === "video" && (
            <button
              onClick={onJoin}
              className="text-[13px] font-semibold text-white bg-blue-600 px-3.5 py-2 rounded-xl flex items-center gap-1.5 active:scale-[0.97] transition-transform"
            >
  ...
  return (
    <div className="rounded-3xl bg-white shadow-[0_1px_4px_rgba(15,23,42,0.06),0_8px_28px_rgba(15,23,42,0.05)] p-5 ring-1 ring-blue-100">
        ...
        {DOCTORS.map((d, i) => (
          <button key={d.name} onClick={() => setDocIdx(i)}
            className={cn("w-full flex items-center gap-3 p-3 rounded-2xl text-left transition-all active:scale-[0.99]",
              docIdx === i ? "bg-blue-50/60 ring-1 ring-blue-200" : "bg-slate-50")}>
            <span className="h-10 w-10 rounded-2xl bg-blue-50 text-blue-600 flex items-center justify-center flex-shrink-0"><Stethoscope className="h-5 w-5" /></span>
        ...
        {DAYS.map((d, i) => (
          <button key={d.key} onClick={() => setDayIdx(i)}
            className={cn("px-3 py-2 rounded-xl text-center transition-all active:scale-[0.97]",
              dayIdx === i ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-700")}>
        ...
        {SLOTS.map(s => (
          <button key={s} onClick={() => setSlot(s)}
            className={cn("px-3 py-1.5 rounded-xl text-[12.5px] font-semibold transition-all active:scale-[0.97]",
              slot === s ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-700")}>
            {s}
          </button>
        ))}
      </div>
      ...
      <button onClick={confirm}
        className="w-full text-[14px] font-semibold text-white bg-blue-600 px-4 py-3 rounded-xl flex items-center justify-center gap-2 active:scale-[0.97] transition-transform">
    ...
    <div className="max-w-4xl mx-auto pb-10 space-y-5">
      <div className="flex items-start justify-between gap-3">
        ...
        <button
          ...
          className="text-[13px] font-semibold text-white bg-blue-600 px-3.5 py-2.5 rounded-xl flex items-center gap-1.5 active:scale-[0.97] transition-transform flex-shrink-0"
        >
