  ...
  return (
    <div className="space-y-5">
      ...
      {ipd.inOT && (
        ...
        >
          <Scissors className="h-5 w-5 text-blue-600 flex-shrink-0 animate-pulse" />
          <div>
            <p className="text-sm font-bold text-blue-900">Surgery in progress</p>
            <p className="text-xs text-blue-700 mt-0.5">{ipd.otProcedure} · Expected completion {ipd.otExpectedEnd}</p>
          </div>
        </motion.div>
      )}
            ...
            return (
              <div key={step.id} className="flex flex-col items-center gap-2 z-10 w-16">
                <div className={cn(
                  ...
                  active ? "bg-blue-50 border-blue-500 ring-4 ring-blue-50" :
                  "bg-white border-slate-200"
                )}>
                  {done
                    ? <CheckCircle className="h-5 w-5 text-green-500" />
                    : <BedDouble className={cn("h-5 w-5", active ? "text-blue-600" : "text-slate-300")} />
                  }
                </div>
      ...
      <div className="p-4 rounded-xl flex items-start gap-3 bg-blue-50 border border-blue-200">
        ...
        <p className="text-sm font-medium leading-relaxed text-blue-800">
          <strong className="text-blue-900">Privacy note:</strong> Clinical details are not shown here. For medical updates, please speak directly with the treating doctor or nursing team.
        </p>
      </div>
    </div>
  )
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="flex gap-1 bg-white rounded-xl border border-slate-200 p-1">
        <button onClick={() => setViewMode('opd')} className={cn("flex-1 py-2 rounded-lg text-sm font-semibold transition-all cursor-pointer", viewMode === 'opd' ? "bg-blue-600 text-white shadow-sm" : "text-slate-500 hover:text-slate-700")}>
          OPD Queue Status
        </button>
        <button onClick={() => setViewMode('ipd')} className={cn("flex-1 py-2 rounded-lg text-sm font-semibold transition-all cursor-pointer", viewMode === 'ipd' ? "bg-blue-600 text-white shadow-sm" : "text-slate-500 hover:text-slate-700")}>
          IPD Patient Tracker
        </button>
      </div>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        <div>
          ...
          <p className="text-sm mt-1 text-slate-500 font-medium">
            Token <strong className="text-blue-600">#{myPatient?.token ?? '—'}</strong> • {myPatient?.department ?? 'General Medicine'} • {myPatient?.doctor ?? 'Your Doctor'}
          </p>
        </div>
        <button
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <div className="hms-card p-6 flex items-center gap-5">
            ...
            <div>
              ...
              <p className="text-xs font-medium text-blue-600 mt-1">Updates in real-time</p>
            </div>
          </div>
        </motion.div>
                ...
                <div key={s.id} className="flex flex-col items-center gap-3 z-10 w-20">
                  <div
                    className={`h-12 w-12 rounded-2xl flex items-center justify-center transition-all duration-300 border-2 ${
                      ...
                      active ? 'bg-blue-50 border-blue-500 shadow-md ring-4 ring-blue-50' : 
                      'bg-white border-slate-200'
                    }`}
                  >
                    <Icon
                      className={`h-5 w-5 ${done ? 'text-green-500' : active ? 'text-blue-600' : 'text-slate-300'}`}
                    />
                  </div>
                  <p
                    className={`text-xs font-bold text-center ${done ? 'text-green-600' : active ? 'text-blue-700' : 'text-slate-400'}`}
                  >
                    {s.label}
                  </p>
                </div>
