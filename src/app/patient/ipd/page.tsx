...
export default function IpdPage() {
  ...
  if (!mounted) return (
    <div className="max-w-3xl mx-auto py-20 flex justify-center">
      <div className="h-8 w-8 rounded-full border-4 border-blue-200 border-t-blue-600 animate-spin" role="status" aria-label="Loading" />
    </div>
  )
  ...
  return (
    <div className="max-w-3xl mx-auto pb-10 space-y-5">
      ...
      {/* Admission header */}
      <div className="rounded-3xl p-5 text-white bg-gradient-to-br from-indigo-600 to-blue-600 shadow-[0_10px_30px_rgba(37,99,235,0.3)]">
          ...
          {PHASES.map((p, i) => (
            <div key={p} className="flex items-center flex-1 last:flex-none">
              <div className="flex flex-col items-center gap-1">
                <div className={cn("h-8 w-8 rounded-full flex items-center justify-center text-[12px] font-bold", i <= phase ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-400")}>
                  {i < phase ? <CheckCircle className="h-4 w-4" /> : i + 1}
                </div>
                <span className={cn("text-[11px] font-semibold", i <= phase ? "text-blue-700" : "text-slate-400")}>{p}</span>
              </div>
              {i < PHASES.length - 1 && <div className={cn("flex-1 h-0.5 mx-1 -mt-4 rounded", i < phase ? "bg-blue-600" : "bg-slate-200")} />}
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="rounded-3xl bg-white shadow-[0_1px_4px_rgba(15,23,42,0.06),0_8px_28px_rgba(15,23,42,0.05)] p-5">
        <h3 className="text-[15px] font-bold text-slate-900 mb-3 flex items-center gap-2"><Stethoscope className="h-4.5 w-4.5 text-blue-600" /> Doctor rounds</h3>
        ...
        <div className="space-y-3">
          {completedRounds.map(r => (
            <div key={r.id} className="flex gap-3">
              <div className="flex flex-col items-center flex-shrink-0"><span className="h-8 w-8 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center"><Stethoscope className="h-4 w-4" /></span><span className="w-px flex-1 bg-slate-100 mt-1" /></div>
              <div className="flex-1 pb-1">
      ...
      {ip.discharge && (() => {
        ...
        return (
          <div className={cn("rounded-3xl p-5 shadow-[0_1px_4px_rgba(15,23,42,0.06),0_8px_28px_rgba(15,23,42,0.05)]", done ? "bg-gradient-to-br from-green-500 to-emerald-600 text-white" : "bg-white")}>
            ...
            {d.followUpDate && (
              <Link href="/patient/followup" className={cn("mt-4 w-full rounded-xl py-2.5 flex items-center justify-center gap-2 font-bold text-[14px] active:scale-[0.98] transition", done ? "bg-white text-green-700" : "bg-blue-600 text-white")}>
