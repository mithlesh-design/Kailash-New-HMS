      ...
      <Card className="p-5">
        <div className="flex items-start justify-between gap-4">
          ...
          {record.claimDocumentsReady && (
            <button className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-blue-50 hover:bg-blue-100 text-blue-700 text-xs font-bold border border-blue-200 transition-colors cursor-pointer flex-shrink-0">
          ...
          {record.medications.map((med, i) => (
            <div key={i} className="flex items-center gap-3 bg-blue-50 rounded-lg px-4 py-3">
              <div className="flex-1">
                ...
                <p className="text-xs text-blue-700 mt-0.5">{med.dose} · {med.frequency} · {med.duration}</p>
              </div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 gap-2">
                      {visit.prescriptions.map((p, j) => (
                        <div key={j} className="flex items-center gap-3 bg-blue-50 rounded-lg px-3 py-2">
                          <Pill className="h-4 w-4 text-blue-600 flex-shrink-0" />
                          <div>
                            <p className="text-sm font-semibold text-blue-800">{p.medicine}</p>
                            <p className="text-xs text-blue-600">{p.dosage} · {p.duration}</p>
                          </div>
                        </div>
                      ))}
                    </div>
