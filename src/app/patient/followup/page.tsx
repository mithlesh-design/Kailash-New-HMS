  ...
  return (
    <div className="max-w-3xl mx-auto pb-10 space-y-5">
      ...
      <div className="rounded-3xl bg-white shadow-[0_1px_4px_rgba(15,23,42,0.06),0_8px_28px_rgba(15,23,42,0.05)] p-5">
        <h3 className="text-[15px] font-bold text-slate-900 mb-3 flex items-center gap-2"><Calendar className="h-4.5 w-4.5 text-blue-600" /> Book your follow-up</h3>
        {!booked ? (
          <div className="flex gap-2">
            <input
              type="date" value={followupDate} min={new Date().toISOString().slice(0, 10)}
              onChange={e => setFollowupDate(e.target.value)}
              className="flex-1 rounded-xl border border-slate-200 px-3.5 py-2.5 text-[14px] text-slate-800 outline-none focus:border-blue-300 focus:ring-2 focus:ring-blue-100"
            />
            <button
              ...
              className="bg-blue-600 text-white font-bold text-[14px] rounded-xl px-4 flex items-center gap-2 active:scale-[0.97] transition"
            >
      ...
      <div className="rounded-3xl bg-white shadow-[0_1px_4px_rgba(15,23,42,0.06),0_8px_28px_rgba(15,23,42,0.05)] p-5">
        <div className="flex items-center justify-between mb-3">
          ...
          {meds.length > 0 && <Link href="/patient/pharmacy" className="text-[12.5px] font-semibold text-blue-600 flex items-center gap-1">Pharmacy <ArrowRight className="h-3.5 w-3.5" /></Link>}
        </div>
        {meds.length === 0 ? (
          <p className="text-[13px] text-slate-500 bg-slate-50 rounded-xl p-3">
            Your medicine schedule will appear here once you review and accept your <Link href="/patient/orders" className="font-semibold text-blue-600">doctor&apos;s orders</Link>.
          </p>
        ) : (
          <div className="space-y-2">
            {meds.map(m => (
      ...
      <div className="grid sm:grid-cols-2 gap-3">
        <Link href="/patient/downloads" className="rounded-3xl bg-white shadow-[0_1px_4px_rgba(15,23,42,0.06),0_8px_28px_rgba(15,23,42,0.05)] p-5 flex items-center gap-3 hover:bg-slate-50 transition">
          <span className="h-10 w-10 rounded-2xl bg-blue-50 flex items-center justify-center flex-shrink-0"><FileText className="h-5 w-5 text-blue-600" /></span>
