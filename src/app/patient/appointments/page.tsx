          ...
          return (
            <button
              ...
              className={cn(
                ...
                isToday ? "text-blue-600 font-bold ring-1 ring-blue-300" :
        ...
        {mode === 'book' && (
          <motion.div key="book" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="space-y-5">
                  ...
                  {SPECIALTIES.map(s => (
                    <button key={s} onClick={() => setSpecialty(s)}
                      className={cn("text-sm font-semibold px-3 py-1.5 rounded-lg border transition-all cursor-pointer",
                        specialty === s ? "bg-blue-600 text-white border-blue-600" : "bg-white text-slate-600 border-slate-200 hover:bg-slate-50"
                      )}>
                      {s}
                    </button>
                  ))}
                ...
                <div className="space-y-3">
                  {filteredDoctors.map(doc => (
                    <button
                      ...
                      className="w-full text-left bg-white border border-slate-200 rounded-xl p-4 hover:border-blue-300 hover:bg-blue-50/20 transition-all cursor-pointer"
                    >
                      <div className="flex items-center justify-between">
            ...
            {step === 'date' && selectedDoctor && (
              <div className="space-y-4">
                <button onClick={() => setStep('doctor')} className="flex items-center gap-1 text-sm text-blue-600 font-semibold cursor-pointer hover:underline">
                  <ChevronLeft className="h-4 w-4" /> Change doctor
                </button>
                <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 flex items-center gap-3">
                  <User className="h-5 w-5 text-blue-600 flex-shrink-0" />
                  <div>
                    <p className="font-bold text-blue-900 text-sm">{selectedDoctor.name}</p>
                    <p className="text-xs text-blue-700">{selectedDoctor.specialty} · ₹{selectedDoctor.fee}</p>
                  </div>
                </div>
                      ...
                      {availableSlots.map(slot => (
                        <button
                          ...
                          className={cn("px-4 py-2 rounded-xl border text-sm font-semibold transition-all cursor-pointer",
                            selectedSlot === slot ? "bg-blue-600 text-white border-blue-600" : "bg-white border-slate-200 text-slate-700 hover:border-blue-300"
                          )}
                        >
                          {slot}
                        </button>
            ...
            {step === 'confirm' && selectedDoctor && selectedSlot && (
              <div className="space-y-4">
                <button onClick={() => setStep('date')} className="flex items-center gap-1 text-sm text-blue-600 font-semibold cursor-pointer hover:underline">
