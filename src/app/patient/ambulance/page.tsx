...
const HISTORY = [
  { date: '08 Apr 2026', route: 'Home, Indiranagar → Kailash Healthcare', type: 'ALS', status: 'Completed' },
  { date: '15 Feb 2026', route: 'Kailash Healthcare → Home, Indiranagar', type: 'BLS', status: 'Completed' },
]
...
export default function AmbulancePage() {
  ...
  const [destination, setDestination] = useState('Kailash Healthcare, MG Road')
    ...
    <div className="max-w-4xl mx-auto pb-10 space-y-5">
      ...
      <div className="rounded-3xl bg-white shadow-[0_1px_4px_rgba(15,23,42,0.06),0_8px_28px_rgba(15,23,42,0.05)] p-5">
        ...
        <div className="space-y-3">
          ...
          <div className="rounded-2xl bg-slate-50 p-3.5 flex items-center gap-3">
            <span className="h-10 w-10 rounded-2xl bg-blue-50 text-blue-600 flex items-center justify-center flex-shrink-0"><Navigation className="h-5 w-5" /></span>
            <div className="flex-1">
            ...
            <div className="grid grid-cols-2 gap-3 mt-1.5">
              {(['BLS', 'ALS'] as AmbType[]).map(t => (
                <button
                  key={t}
                  onClick={() => setType(t)}
                  className={cn("rounded-2xl p-3 text-left transition-colors", type === t ? "bg-blue-600 text-white" : "bg-slate-50 text-slate-700")}
                >
        ...
        <div className="rounded-3xl bg-white shadow-[0_1px_4px_rgba(15,23,42,0.06),0_8px_28px_rgba(15,23,42,0.05)] p-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="rounded-2xl bg-slate-50 p-3.5">
              ...
              <p className="text-[14px] font-bold text-slate-900 flex items-center gap-1.5"><Clock className="h-4 w-4 text-blue-600" /> 8 min</p>
            </div>
          </div>
