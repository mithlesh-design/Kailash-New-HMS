      ...
      {!disabled && (
        <div className="flex gap-2">
          <input value={draft} onChange={e => setDraft(e.target.value)}
            ...
            className="flex-1 h-8 px-2 rounded-md ring-1 ring-slate-200 text-[12.5px] focus:outline-none focus:ring-blue-400" />
          <button onClick={() => { if (draft.trim()) { onChange([...values, draft.trim()]); setDraft('') } }}
            className="h-8 w-8 rounded-md bg-blue-600 text-white flex items-center justify-center hover:bg-blue-700 cursor-pointer">
    ...
    <div className="max-w-4xl mx-auto pb-10 space-y-5">
      <div className="flex items-end justify-between gap-3 flex-wrap">
        ...
        {profile?.completedAt && !editing && (
          <button onClick={startEdit} className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-[12.5px] font-semibold cursor-pointer">
        ...
        <>
          ...
          <div className="grid md:grid-cols-2 gap-3">
            ...
            <Card title="Emergency contact" icon={Phone}>
              {editing && draft ? (
                <div className="space-y-1.5">
                  <input value={draft.emergencyName ?? ''} onChange={e => setDraft({ ...draft, emergencyName: e.target.value })} placeholder="Name" className="w-full h-8 px-2 rounded-md ring-1 ring-slate-200 text-[12.5px] focus:outline-none focus:ring-blue-400" />
                  <input value={draft.emergencyRelation ?? ''} onChange={e => setDraft({ ...draft, emergencyRelation: e.target.value })} placeholder="Relation" className="w-full h-8 px-2 rounded-md ring-1 ring-slate-200 text-[12.5px] focus:outline-none focus:ring-blue-400" />
                  <input value={draft.emergencyPhone ?? ''} onChange={e => setDraft({ ...draft, emergencyPhone: e.target.value })} placeholder="Phone" className="w-full h-8 px-2 rounded-md ring-1 ring-slate-200 text-[12.5px] focus:outline-none focus:ring-blue-400" />
                </div>
              ) : (
                <>
            ...
            <Card title="Contact & address" icon={MapPin}>
              {editing && draft ? (
                <div className="space-y-1.5">
                  <input value={draft.address ?? ''} onChange={e => setDraft({ ...draft, address: e.target.value })} placeholder="Address" className="w-full h-8 px-2 rounded-md ring-1 ring-slate-200 text-[12.5px] focus:outline-none focus:ring-blue-400" />
                  <div className="flex gap-1.5">
                    <input value={draft.city ?? ''} onChange={e => setDraft({ ...draft, city: e.target.value })} placeholder="City" className="flex-1 h-8 px-2 rounded-md ring-1 ring-slate-200 text-[12.5px] focus:outline-none focus:ring-blue-400" />
                    <input value={draft.pincode ?? ''} onChange={e => setDraft({ ...draft, pincode: e.target.value })} placeholder="Pincode" className="w-24 h-8 px-2 rounded-md ring-1 ring-slate-200 text-[12.5px] focus:outline-none focus:ring-blue-400" />
                  </div>
                </div>
              ) : (
                <>
