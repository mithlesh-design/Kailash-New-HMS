...
const STATUS_CONFIG: Record<RefundStatus, { label: string; color: string; icon: React.ElementType }> = {
  ...
  approved_lead:     { label: 'Awaiting Finance',     color: 'bg-blue-100 text-blue-700',    icon: Shield },
  approved_finance:  { label: 'Ready for payout',     color: 'bg-indigo-100 text-indigo-700', icon: CheckCircle },
    ...
    <div className="space-y-6 pt-6">
      <div className="flex items-start justify-between gap-3 flex-wrap">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-[11px] font-semibold">
          ...
          <Pill label="Awaiting Finance" value={totals.awaitingFh} tint="bg-blue-50 text-blue-700" />
          <Pill label="Ready for payout" value={totals.ready} tint="bg-indigo-50 text-indigo-700" />
                ...
                <div className="flex flex-col items-end gap-2 flex-shrink-0">
                  ...
                  <div className="flex gap-2 flex-wrap justify-end">
                    {r.status === 'pending' && canLeadApprove && (
                      <button onClick={() => leadApprove(r)} className="px-3 py-1.5 text-xs font-bold bg-blue-600 text-white rounded-lg hover:bg-blue-700">Approve 1/2</button>
                    )}
                    {r.status === 'approved_lead' && canFinanceApprove && (
                      <button onClick={() => financeApprove(r)} className="px-3 py-1.5 text-xs font-bold bg-indigo-600 text-white rounded-lg hover:bg-indigo-700">Authorise 2/2</button>
                    )}
