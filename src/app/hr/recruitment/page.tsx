...
const STAGE_STYLE: Record<ApplicantStage, string> = {
  ...
  Screening: 'border-blue-200 bg-blue-50/60',
  Interview: 'border-indigo-200 bg-indigo-50/60',
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2"><Workflow className="h-6 w-6 text-indigo-600" /> Recruitment</h1>
          <p className="text-sm text-slate-500 mt-1">{openPositions} open positions · {inPipeline} candidates in pipeline</p>
        </div>
        <Link href="/hr/onboarding" className="inline-flex items-center gap-1.5 text-sm font-semibold text-blue-600 hover:text-blue-800">Onboarding <ArrowRight className="h-4 w-4" /></Link>
      </div>
                    ...
                    <div key={a.id} className="rounded-xl bg-white border border-slate-200 p-2.5 shadow-sm">
                      ...
                      {stage !== 'Hired' && stage !== 'Rejected' && (
                        <div className="flex items-center gap-1 mt-1.5">
                          {next && <button onClick={() => move(a.id, next)} className="flex-1 text-[10px] font-bold px-1.5 py-1 rounded-md bg-blue-600 hover:bg-blue-700 text-white cursor-pointer">→ {next}</button>}
