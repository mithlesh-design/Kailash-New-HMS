...
export default function DiscoveryPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      {/* Hero */}
      <div className="bg-gradient-to-br from-blue-600 to-indigo-700 text-white py-10 px-6">
        <div className="max-w-3xl mx-auto">
          <p className="text-blue-200 text-sm font-semibold mb-1">Kailash Hospital</p>
          <h1 className="text-3xl font-bold mb-2">Find the Right Doctor</h1>
          <p className="text-blue-200 text-sm mb-6">Search by symptom, specialty, or doctor name</p>
          ...
          <AnimatePresence>
            {specialtyMatches.length > 0 && (
              <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="mt-3 flex items-center gap-2 flex-wrap">
                <span className="text-blue-200 text-xs font-semibold">Suggested:</span>
            ...
            <div className="space-y-3">
              {filteredDoctors.map((doc, i) => (
                <motion.div key={doc.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
                  <div className="bg-white rounded-2xl border border-slate-200 p-5 hover:border-blue-200 transition-colors">
                    <div className="flex items-start gap-4">
                      <div className="h-12 w-12 rounded-full bg-gradient-to-br from-blue-50 to-indigo-100 border border-blue-100 flex items-center justify-center flex-shrink-0">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center justify-between mt-3">
                          ...
                          <Link href="/patient/appointments">
                            <button className="text-xs font-bold bg-blue-600 hover:bg-blue-700 text-white px-4 py-1.5 rounded-xl cursor-pointer transition-colors flex items-center gap-1">
                ...
                <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
                  {insuerer?.cashless ? (
                    <div className="flex items-start gap-3 p-5 bg-green-50 border border-green-200 rounded-2xl">
                      ...
                      <div>
                        <p className="font-bold text-green-900">Cashless available at Kailash Hospital</p>
                  ...
                  <div className="mt-3 p-4 bg-blue-50 border border-blue-200 rounded-xl">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <p className="text-sm font-bold text-blue-900">Insurance Helpdesk</p>
                    </div>
                    <p className="text-xs text-blue-700">For pre-authorization and TPA queries, visit the Insurance Desk (Ground Floor) or call Ext. 1050</p>
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
                ...
                {Object.keys(DOCS_BY_TYPE).map(vt => (
                  <button
                    ...
                    className={cn("text-sm font-semibold px-3 py-3 rounded-xl border cursor-pointer transition-all text-left",
                      selectedVisitType === vt ? "bg-blue-600 text-white border-blue-600" : "bg-white text-slate-700 border-slate-200 hover:bg-slate-50"
                    )}
                  >
                    {vt}
                  </button>
              ...
              <ul className="space-y-3">
                {DOCS_BY_TYPE[selectedVisitType].map((doc, i) => (
                  <li key={i} className="flex items-start gap-3">
                    <div className="h-5 w-5 rounded-full bg-blue-50 border border-blue-200 flex items-center justify-center flex-shrink-0 mt-0.5">
