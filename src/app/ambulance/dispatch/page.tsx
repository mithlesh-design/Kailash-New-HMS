    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
        ...
        <div className="flex gap-3 mt-5">
          ...
          <button onClick={submit} className="flex-1 h-10 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold cursor-pointer">Save</button>
        </div>
      </motion.div>
    </motion.div>
  )
  ...
  return (
    <div className="space-y-6 pt-6">
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-5">
        <h3 className="font-bold text-slate-800 mb-3 flex items-center gap-2"><MapPin className="h-4 w-4 text-blue-500" /> Active trips ({activeTrips.length})</h3>
                ...
                <div key={t.id} className="rounded-xl bg-slate-50 p-3 flex items-center gap-3 flex-wrap">
                  ...
                  {next && (
                    <button onClick={() => advance(t.id)}
                      className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[11.5px] font-semibold bg-blue-600 hover:bg-blue-700 text-white cursor-pointer">
                      → {next.replace('_', ' ')}
                    </button>
                  )}
                </div>
        ...
        <div className="space-y-2">
          {vehicles.map(v => (
            <div key={v.id} className="rounded-xl bg-slate-50 p-3 flex items-center gap-3 flex-wrap">
              ...
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full capitalize ${v.status === 'available' ? 'bg-green-100 text-green-700' : v.status === 'on_trip' ? 'bg-blue-100 text-blue-700' : 'bg-slate-200 text-slate-700'}`}>{v.status.replace('_', ' ')}</span>
