      ...
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
            <ShieldCheck className="h-6 w-6 text-indigo-600" />Coverage Requirements
          </h1>
        ...
        {canWrite && (
          <button onClick={() => setShowAdd(true)}
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold bg-indigo-600 hover:bg-indigo-700 text-white cursor-pointer">
                ...
                <motion.tr key={dept.department}
                  ...
                  <td className="px-4 py-3">
                    <span className={cn('text-[10px] font-bold uppercase px-1.5 py-0.5 rounded',
                      dept.perShift ? 'bg-blue-100 text-blue-700' : 'bg-slate-100 text-slate-600')}>
                      ...
                      isEditing ? (
                        ...
                        <div className="flex gap-1">
                          <button onClick={() => setEditing(dept.department)}
                            className="flex items-center gap-1 px-2 py-1 rounded-lg bg-indigo-50 hover:bg-indigo-100 text-indigo-700 text-[11px] font-bold cursor-pointer">
        ...
        <motion.div
          ...
          <div onClick={(e) => e.stopPropagation()}
            ...
            <h3 className="text-base font-bold text-slate-900 mb-1 flex items-center gap-2">
              <Plus className="h-4 w-4 text-indigo-600" />Add coverage requirement
            </h3>
                  ...
                  {ALL_ROLES.filter(r => r !== 'patient').map(r => (
                    <button key={r} type="button" onClick={() => {
                      ...
                      className={cn('text-[10px] font-bold uppercase px-1.5 py-0.5 rounded cursor-pointer',
                        newDept.roles.includes(r as Role) ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200')}>
                      {r.replace('_', ' ')}
                    </button>
                  ))}
                </div>
