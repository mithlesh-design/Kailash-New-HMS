...
const KPI = [
  {
    ...
    trend: '+12%', up: true, icon: Users, color: 'text-blue-600',
    cardBg: 'bg-blue-50/70', iconBg: 'bg-white', labelColor: 'text-blue-800/60', ringColor: '#2563EB', ring: 68,
  },
  {
                    ...
                    <div key={p.id} className="flex items-center gap-3 p-2 bg-slate-50 rounded-lg">
                      ...
                      <div className="text-right flex-shrink-0">
                        ...
                        <div className="h-1.5 w-20 bg-slate-200 rounded-full overflow-hidden mt-1">
                          <div className="h-full bg-blue-500 rounded-full" style={{ width: `${(cleared / total) * 100}%` }} />
                        </div>
                      </div>
                    </div>
                  )
                  ...
                  return (
                    <div key={bill.id} className="flex items-center gap-4 p-3 bg-slate-50 rounded-xl">
                      ...
                      <span className={cn("text-xs font-bold px-2 py-1 rounded-lg border flex-shrink-0",
                        ...
                        bill.status === 'frozen' ? 'bg-blue-50 text-blue-700 border-blue-200' :
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              ...
              return (
                <div key={day} className="flex-1 flex flex-col items-center justify-end gap-2 group">
                  ...
                  <motion.div
                    ...
                    className={`w-full max-w-[48px] rounded-t-lg transition-all ${isPeak ? 'bg-gradient-to-t from-blue-500 to-blue-400' : 'bg-slate-100 hover:bg-blue-100'}`}
                    style={{ minHeight: 4 }}
                  />
                  <span className={`text-xs font-semibold mt-2 ${isPeak ? 'text-blue-600' : 'text-slate-500'}`}>
                    {day}
                  </span>
                </div>
              )
        ...
        >
          ...
          <div className="p-3 bg-white border-t border-slate-100 text-center">
            <button onClick={() => router.push('/admin/patients')} className="text-sm font-semibold text-blue-600 hover:text-blue-700 cursor-pointer">View All Patients</button>
          </div>
        </motion.div>
        </Card>
