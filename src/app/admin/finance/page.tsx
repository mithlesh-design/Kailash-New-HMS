    ...
    <div className="space-y-5 p-6 max-w-7xl mx-auto">
      <div className="flex items-start justify-between gap-3 flex-wrap">
        ...
        <div className="flex gap-2 items-center">
          ...
          <Link href="/admin/payroll" className="flex items-center gap-1 text-xs font-bold text-indigo-700 hover:underline">
            Payroll →
          </Link>
          <Link href="/admin/vendors" className="flex items-center gap-1 text-xs font-bold text-indigo-700 hover:underline">
            Vendors →
          </Link>
          <Link href="/admin/disputes" className="flex items-center gap-1 text-xs font-bold text-indigo-700 hover:underline">
            Disputes →
          </Link>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        <KPI label="Revenue" value={fmtINRk(revenueMetrics.totalRevenue)} sub={`${bills.length} bills`} tint="bg-blue-50 border-blue-200 text-blue-700" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4">
          <h3 className="text-sm font-bold text-slate-800 mb-3 flex items-center gap-2">
            <BarChart3 className="h-4 w-4 text-blue-600" />Revenue by source · {new Date(month + '-01').toLocaleDateString('en-IN', { month: 'long' })}
          </h3>
          {(() => {
                ...
                {sorted.map(([dept, amt]) => (
                  <div key={dept}>
                    ...
                    <div className="h-1.5 mt-0.5 bg-slate-100 rounded-full overflow-hidden">
                      <div className="h-full bg-blue-500" style={{ width: `${(amt / max) * 100}%` }} />
                    </div>
                  </div>
                ))}
              </div>
