...
export default function AdminDoctorActivity() {
  ...
  return (
    <div className="max-w-5xl mx-auto pb-8">
      <div className="mb-4">
        <h1 className="text-[24px] font-bold text-slate-900 tracking-tight flex items-center gap-2"><Stethoscope className="h-6 w-6 text-blue-600" /> Doctor Activity</h1>
              ...
              <tr key={doc.id} className="border-b border-slate-50 hover:bg-slate-50/60 transition">
                ...
                {COLS.map(c => (
                  <td key={c.key} className="py-3 px-2 text-center">
                    <span className={cn("text-[14px] font-bold tabular-nums", c.key === 'consults' ? "text-blue-700" : "text-slate-800")}>{t[c.key].toLocaleString('en-IN')}</span>
                  </td>
                ))}
              </tr>
            ))}
