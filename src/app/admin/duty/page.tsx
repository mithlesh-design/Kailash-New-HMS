      ...
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
            <ClipboardList className="h-6 w-6 text-indigo-600" />Duty Assignment
          </h1>
                    ...
                    {canWrite && (
                      duty ? (
                        ...
                        <button onClick={() => { setAssignTarget({ staffId: member.id, staffName: member.name }); setSelectedWard('General Ward') }}
                          className="text-[11px] font-bold text-indigo-600 hover:underline cursor-pointer">Assign</button>
                      ) : (
              ...
              {WARDS.map(w => (
                <button key={w} onClick={() => setSelectedWard(w)}
                  className={cn('text-[11px] font-bold px-2 py-2 rounded-lg border cursor-pointer transition',
                    selectedWard === w ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-slate-700 border-slate-200 hover:bg-slate-50')}>
                  {w}
                </button>
              ))}
            </div>
