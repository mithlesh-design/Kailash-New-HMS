      ...
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
            <Calendar className="h-6 w-6 text-indigo-600" />Staff Roster
          </h1>
      ...
      {(conflictsSummary.critical > 0 || conflictsSummary.warning > 0) && (
        <div className="rounded-xl border border-slate-200 bg-white p-3 flex items-center gap-3 flex-wrap">
          ...
          <button onClick={() => setShowConflicts(v => !v)}
            className="ml-auto flex items-center gap-1 text-[11px] font-bold text-indigo-700 hover:underline cursor-pointer">
              ...
              {dateRange.map(date => {
                ...
                return (
                  <th key={date} className={cn('text-center px-1.5 py-2 font-bold text-slate-600',
                    ...
                    <div className={cn('text-[10px]', isToday ? 'text-blue-700 font-extrabold' : 'text-slate-500')}>
                      {formatDayShort(date)}
                    </div>
                    <div className={cn('text-xs', isToday ? 'text-blue-700' : 'text-slate-700')}>{formatDayNum(date)}</div>
                    {hasAlert && (
            ...
            {filteredStaff.map((member, rowIdx) => (
              <motion.tr key={member.id}
                ...
                className={cn('border-b border-slate-100 last:border-0 hover:bg-slate-50/50',
                  selectedRows.has(member.id) && 'bg-indigo-50/30')}>
