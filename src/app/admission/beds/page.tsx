...
const BED_STATUS_STYLE: Record<string, { bg: string; border: string; text: string; dot: string }> = {
  ...
  Occupied: { bg: 'bg-blue-50', border: 'border-blue-200', text: 'text-blue-800', dot: 'bg-blue-500' },
  Cleaning: { bg: 'bg-yellow-50', border: 'border-yellow-200', text: 'text-yellow-800', dot: 'bg-yellow-500' },
  Reserved: { bg: 'bg-blue-50', border: 'border-blue-200', text: 'text-blue-800', dot: 'bg-blue-500' },
    ...
    >
      ...
      {bed.status === 'Occupied' && bed.occupantName && (
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-semibold text-blue-800 truncate">{bed.occupantName}</span>
        </div>
      )}
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 text-center">
          ...
          <p className="text-xs font-semibold text-blue-600 mt-1">Occupied ({Math.round(stats.occupied / stats.total * 100)}%)</p>
        </div>
          ...
          {['All', ...WARD_ORDER].map(ward => (
            <button
              ...
              className={cn(
                ...
                filterWard === ward ? "bg-blue-600 text-white" : "bg-white border border-slate-200 text-slate-600 hover:bg-slate-50"
              )}
            >
              {ward}
            </button>
