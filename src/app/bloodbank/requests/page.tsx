              ...
              <button onClick={() => setOpen(isOpen ? null : req.id)}
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-bold text-slate-900 flex items-center gap-1.5 flex-wrap">
                    ...
                    {req.status === 'issued' && <span className="text-[10px] font-bold uppercase px-1.5 py-0.5 rounded bg-indigo-100 text-indigo-700">Issued</span>}
                  </p>
                    ...
                    <>
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => onIssue(req)} disabled={!allChecked}
                          className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold bg-indigo-600 hover:bg-indigo-700 text-white cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed">
                          ...
                          return (
                            <div key={uid} className="rounded-lg bg-white border border-indigo-200 p-2.5 text-xs">
                              <p className="font-bold text-slate-800 flex items-center gap-1.5">
                                <Clock className="h-3 w-3 text-indigo-600" />Bag {u.bagNumber} → {req.patientName} ({req.patientId})
                              </p>
