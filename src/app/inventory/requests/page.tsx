      ...
      <Section title="Pending action" count={pending.length} emptyText="No requests waiting to be placed.">
        {pending.map(po => (
          <RequestRow key={po.id} po={po} action={
            <button onClick={() => markOrdered(po)} className="flex items-center gap-1.5 text-xs font-bold text-blue-700 bg-blue-50 hover:bg-blue-100 px-3 py-1.5 rounded-lg cursor-pointer"><Truck className="h-3.5 w-3.5" />Mark ordered</button>
          } />
        ))}
      </Section>
