...
import { FileText, CheckCircle, XCircle, Clock, AlertCircle, X, Sparkles, Loader2, Send, ChevronDown, ChevronUp, ShieldCheck } from "lucide-react"
...
export default function InsuranceClaimsPage() {
  const { claims, setValidation, setSubmissionStatus } = useInsuranceStore()
  ...
  const handleValidate = async (claim: InsuranceClaim) => {
    ...
    setLocalClaims(prev => prev.map(c => c.id === claim.id ? { ...c, aiValidation: result.data, submissionStatus: 'validated' } : c))
