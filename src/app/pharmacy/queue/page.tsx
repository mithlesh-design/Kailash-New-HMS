      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="relative flex-1 min-w-[200px] max-w-sm">
          ...
          <input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search patient, doctor, drug, source…"
            className="w-full pl-9 pr-3 py-2 text-sm rounded-xl border border-slate-200 bg-white focus:outline-none focus:ring-2 focus:ring-pink-200" />
        </div>
      </div>
          ...
          <div className="flex-shrink-0 flex items-center gap-2 flex-wrap justify-end">
            ...
            {["Self (patient)", "Relative", "Ward nurse"].map(c => (
              <button key={c} onClick={() => props.setCollector(c)}
                className={cn("text-[11px] font-semibold px-2 py-1 rounded-lg cursor-pointer", props.collector === c ? "bg-pink-100 text-pink-700" : "bg-slate-100 text-slate-600 hover:bg-slate-200")}>{c}</button>
            ))}
            <input value={props.collector} onChange={e => props.setCollector(e.target.value)}
              className="w-32 px-2 py-1 text-xs rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-pink-200" />
                  ...
                  <div className="flex items-center justify-between gap-3 flex-wrap">
                    <div className="min-w-0">
                      <p className="text-sm font-semibold text-slate-800 flex items-center gap-1.5">
                        <Pill className="h-3.5 w-3.5 text-pink-500" />{m.name}
