...
const STATUS_STYLE: Record<string, { tint: string; dot: string }> = {
  ...
  Occupied:    { tint: 'bg-blue-50 text-blue-700 border-blue-100',      dot: 'bg-blue-500' },
  Cleaning:    { tint: 'bg-amber-50 text-amber-700 border-amber-100',   dot: 'bg-amber-500' },
  Reserved:    { tint: 'bg-violet-50 text-violet-700 border-violet-100',dot: 'bg-violet-500' },
...
const ETA_TONE: Record<string, string> = {
  ...
  red: 'bg-red-100 text-red-700', violet: 'bg-violet-100 text-violet-700', slate: 'bg-slate-100 text-slate-500',
}
...
export default function ReceptionBeds() {
  ...
  const tiles = [
    ...
    { label: 'Occupied', value: count('Occupied'), icon: User, tint: 'bg-blue-50 text-blue-600' },
              ...
              {freeing.map(b => {
                ...
                return (
                  <div key={b.id} className="flex items-center gap-3 rounded-xl bg-slate-50 px-3 py-2.5">
                    <span className={cn("h-9 w-9 rounded-xl flex items-center justify-center flex-shrink-0", b.status === 'Cleaning' ? "bg-amber-100 text-amber-600" : "bg-blue-50 text-blue-600")}>
