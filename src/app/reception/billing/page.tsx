...
export default function ReceptionBilling() {
  ...
  const tiles = [
    ...
    { label: 'Total billed', value: `₹${totalBilled.toLocaleString('en-IN')}`, icon: CreditCard, tint: 'bg-blue-50 text-blue-600' },
  ]
        ...
        <div className="flex gap-1">
          {(['all', 'pending', 'settled'] as const).map(f => (
            <button key={f} onClick={() => setFilter(f)}
              className={cn("text-[12px] font-bold px-3 py-1.5 rounded-lg capitalize transition", filter === f ? "bg-blue-600 text-white" : "bg-white border border-slate-200 text-slate-600 hover:bg-slate-50")}>{f}</button>
          ))}
        </div>
      </div>
