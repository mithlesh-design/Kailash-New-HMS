...
const STATUS_TINT: Record<string, string> = {
  upcoming: 'bg-blue-50 text-blue-700', confirmed: 'bg-green-50 text-green-700', cancelled: 'bg-red-50 text-red-600',
}
  ...
  return (
    <div className="pb-6">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
        ...
        <button onClick={openNew} className="flex items-center gap-2 h-10 px-4 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-[13.5px] font-bold shadow-sm active:scale-[0.98] transition">
        ...
        <div className="flex gap-1">
          {(['all', 'upcoming', 'confirmed', 'cancelled'] as const).map(s => (
            <button key={s} onClick={() => setStatusFilter(s)}
              className={cn("text-[11.5px] font-bold px-3 py-1.5 rounded-lg capitalize transition", statusFilter === s ? "bg-blue-600 text-white" : "bg-white border border-slate-200 text-slate-600 hover:bg-slate-50")}>{s}</button>
          ))}
        </div>
      </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }}
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input list="rc-patient-list" value={draft.patientName} onChange={e => setDraft(d => ({ ...d, patientName: e.target.value }))} placeholder="Search or type a name"
                      className="w-full h-10 pl-9 pr-3 rounded-xl border border-slate-200 text-sm text-slate-900 outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-300" />
                  ...
                  <div className="flex gap-2">
                    {([['in_person', 'In-person', Building2], ['online', 'Online', Video]] as const).map(([key, label, Icon]) => (
                      <button key={key} onClick={() => setDraft(d => ({ ...d, mode: key }))}
                        className={cn("flex-1 h-10 rounded-xl text-sm font-semibold flex items-center justify-center gap-1.5 transition", draft.mode === key ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200")}>
              ...
              <div className="flex gap-3 mt-6">
                ...
                <button onClick={submit} className="flex-1 h-11 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-[13.5px] flex items-center justify-center gap-2 transition">
