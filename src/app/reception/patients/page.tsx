      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
        ...
        <div className="relative w-full sm:w-72">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search name, ID or phone…"
            className="w-full h-10 pl-9 pr-3 rounded-xl bg-white border border-slate-200 text-[14px] text-slate-800 placeholder:text-slate-400 outline-none focus:border-blue-300 focus:ring-2 focus:ring-blue-100" />
        </div>
      </div>
      ...
      <div className="flex items-center gap-1 p-1 rounded-xl bg-slate-100 w-fit mb-3">
        {TABS.map(t => (
          <button key={t} onClick={() => setTab(t)}
            ...
            {t} <span className={cn("text-[11px] font-bold px-1.5 rounded-full", tab === t ? "bg-blue-100 text-blue-700" : "bg-slate-200 text-slate-500")}>{counts[t]}</span>
          </button>
        ))}
      </div>
        ...
        <div className="flex gap-1">
          {['All', 'Critical', 'High', 'Medium', 'Low'].map(t => (
            <button key={t} onClick={() => setTriage(t)}
              className={cn("text-[11px] font-bold px-2.5 py-1.5 rounded-lg transition", triage === t ? "bg-blue-600 text-white" : "bg-white border border-slate-200 text-slate-600 hover:bg-slate-50")}>{t}</button>
          ))}
        </div>
            ...
            return (
              <button key={p.id} onClick={() => setSelectedId(p.id)}
                ...
                <div className="flex items-start gap-3">
                  <span className="h-11 w-11 rounded-2xl bg-gradient-to-br from-blue-500 to-indigo-500 text-white flex items-center justify-center font-bold text-[14px] flex-shrink-0">{initials(p.name)}</span>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                      ...
                      {tab === 'Upcoming' && appt && <span className="text-[11px] font-semibold text-indigo-600 flex items-center gap-1"><Calendar className="h-3 w-3" /> {new Date(appt.date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })} {appt.time}</span>}
    ...
    <div>
      ...
      <div className="sticky top-0 bg-white border-b border-slate-100 px-5 py-4 flex items-start justify-between gap-3 z-10">
        <div className="flex items-center gap-3">
          <span className="h-12 w-12 rounded-2xl bg-gradient-to-br from-blue-500 to-indigo-500 text-white flex items-center justify-center font-bold text-[16px]">{initials(p.name)}</span>
          <div>
      ...
      <div className="p-5 space-y-5">
        ...
        <Section title="Chief complaint">
          {p.symptoms.length ? <div className="flex flex-wrap gap-1.5">{p.symptoms.map(s => <span key={s} className="text-[12px] font-medium bg-blue-50 text-blue-700 px-2.5 py-1 rounded-full">{s}</span>)}</div> : <p className="text-[12.5px] text-slate-400">None recorded</p>}
        </Section>
        {p.history.length > 0 && (
            ...
            <div className="space-y-2">
              {appointments.map(a => (
                <div key={a.id} className="flex items-center gap-2.5 rounded-xl bg-slate-50 p-2.5">
                  ...
                  <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full capitalize", a.status === 'cancelled' ? 'bg-red-50 text-red-600' : a.status === 'confirmed' ? 'bg-green-50 text-green-700' : 'bg-blue-50 text-blue-700')}>{a.status}</span>
                </div>
              ))}
            </div>
          </Section>
      ...
      <div className="sticky bottom-0 bg-white border-t border-slate-100 p-4 flex gap-2">
        ...
        {next && <button onClick={onAdvance} className="flex-1 h-11 rounded-xl bg-blue-600 text-white font-bold text-[13.5px] flex items-center justify-center gap-2 hover:bg-blue-700 transition">{next.label} <ArrowRight className="h-4 w-4" /></button>}
      </div>
    </div>
  )
}
