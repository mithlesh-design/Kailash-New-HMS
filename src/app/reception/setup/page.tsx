...
function Toggle({ on, onToggle }: { on: boolean; onToggle: () => void }) {
  return (
    <button onClick={onToggle} aria-pressed={on} className={cn("h-6 w-11 rounded-full transition-colors flex-shrink-0 relative", on ? "bg-blue-600" : "bg-slate-200")}>
  ...
  return (
    <div className="max-w-2xl mx-auto pb-6">
      ...
      <div className="space-y-4">
        <div className={CARD}>
          <h3 className="text-[15px] font-bold text-slate-900 mb-3 flex items-center gap-2"><Building2 className="h-4.5 w-4.5 text-blue-600" /> This counter</h3>
        ...
        <button onClick={save} className="w-full h-11 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-[14px] flex items-center justify-center gap-2 active:scale-[0.99] transition">
