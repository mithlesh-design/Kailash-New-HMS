...
const STYLE: Record<Cat, { Icon: typeof FileText; tint: string }> = {
  ...
  'Token slips': { Icon: Ticket, tint: 'bg-violet-50 text-violet-600' },
      ...
      <div className="rounded-2xl bg-white shadow-[0_1px_4px_rgba(15,23,42,0.06),0_4px_16px_rgba(15,23,42,0.04)] p-5 mb-4">
        <div className="relative">
          ...
          <input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search documents…"
            className="w-full rounded-2xl bg-slate-50 border border-slate-100 pl-11 pr-4 py-3 text-[14px] text-slate-800 placeholder:text-slate-400 outline-none focus:border-blue-200 focus:bg-white transition" />
        </div>
        <div className="mt-4 flex flex-wrap gap-2">
          {FILTERS.map(f => (
            <button key={f} onClick={() => setFilter(f)}
              className={cn("text-[12.5px] font-semibold px-3.5 py-1.5 rounded-full transition", filter === f ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200")}>{f}</button>
          ))}
        </div>
      </div>
