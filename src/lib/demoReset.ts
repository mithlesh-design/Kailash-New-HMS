...
//
// Wipes every persisted Zustand store (kailash-* keys in localStorage) so the
...
const DEMO_KEY_PREFIX = 'kailash-'
/** Keys we deliberately keep across resets (no patient data in them). */
const KEEP_KEYS = new Set<string>([
  'kailash-authstore',           // keep the user logged in
  'kailash-doctor-profile',      // keep the doctor's settings
])
