...
const WHATSAPP_TEMPLATES: Partial<Record<string, (n: Notification) => string>> = {
  bed_allocated: (n) =>
    `Dear ${n.patientName ?? 'Patient'}, your bed has been allocated at Kailash Hospital. Please proceed to the ward as directed. — Kailash Hospital`,
  ot_confirmed: (n) =>
    `Dear ${n.patientName ?? 'Patient'}, your surgery has been scheduled. Please follow pre-operative instructions. — Kailash Hospital`,
  medicines_ready: (n) =>
    `Dear ${n.patientName ?? 'Patient'}, your medicines are ready for collection at the ward pharmacy counter. — Kailash Hospital`,
  discharge_initiated: (n) =>
    `Dear ${n.patientName ?? 'Patient'}, your discharge process has been initiated. Please visit the billing counter with your documents. — Kailash Hospital`,
  discharge_ready: (n) =>
    `Dear ${n.patientName ?? 'Patient'}, your discharge summary is ready. Please collect it from the nurse's station. — Kailash Hospital`,
  followup_reminder: (n) =>
    `Dear ${n.patientName ?? 'Patient'}, this is a reminder for your follow-up visit at Kailash Hospital. ${n.body} — Kailash Hospital`,
}

const SMS_TEMPLATES: Partial<Record<string, (n: Notification) => string>> = {
  bed_allocated: (n) => `Kailash Hospital: Bed allocated for ${n.patientName ?? 'you'}. Proceed to ward.`,
  ot_confirmed: (n) => `Kailash Hospital: Surgery confirmed for ${n.patientName ?? 'you'}. Follow pre-op instructions.`,
  medicines_ready: (n) => `Kailash Hospital: Medicines ready for ${n.patientName ?? 'you'}. Collect from ward pharmacy.`,
  discharge_initiated: (n) => `Kailash Hospital: Discharge started for ${n.patientName ?? 'you'}. Visit billing counter.`,
  followup_reminder: (n) => `Kailash Hospital: Follow-up reminder — ${n.body}`,
}
...
function buildMessage(notification: Notification, channel: NotificationChannel): string {
  ...
  return channel === 'whatsapp'
    ? `Kailash Hospital: ${notification.title} — ${notification.body}`
    : `Kailash Hospital: ${notification.body}`
}
