...
export function DrugSafetyReasoningCard({ meds, allergies, comorbidities, onAccept, onSubstitute, className }: Props) {
  ...
  return (
    <div className={cn("rounded-2xl bg-white ring-1 ring-slate-200/80 shadow-sm overflow-hidden", className)}>
      ...
      {alternates.length > 0 ? (
        <div className="border-t border-slate-100 px-4 py-3 bg-blue-50/30">
          <div className="flex items-center gap-2 mb-2">
            <Sparkles className="h-3.5 w-3.5 text-blue-600" />
            <p className="text-[12px] font-semibold text-blue-900">AI-suggested alternatives</p>
            <span className="ml-auto text-[10px] font-mono text-blue-700">82% confidence</span>
          </div>
          {showAlternates ? (
            <div className="space-y-2">
              {alternates.map((alt, i) => (
                <div key={i} className="rounded-lg bg-white ring-1 ring-blue-100/80 px-3 py-2 flex items-start gap-2">
                  <Pill className="h-3.5 w-3.5 text-blue-600 mt-0.5 flex-shrink-0" />
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1 px-2 py-1 rounded-md bg-blue-600 hover:bg-blue-700 text-white text-[11px] font-semibold flex-shrink-0"
                  >
