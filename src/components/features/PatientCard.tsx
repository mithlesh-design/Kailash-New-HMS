...
const statusConfig: Record<QueueStatus, { label: string; color: string; bg: string; border: string }> = {
  ...
  pharmacy:   { label: 'Pharmacy',   color: 'text-blue-600', bg: 'bg-blue-50', border: 'border-blue-200' },
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        compact && "p-3",
        selected ? "border-blue-500 shadow-md ring-1 ring-blue-500/20" : "border-slate-200 hover:border-slate-300 hover:shadow-md"
      )}
    >
      {selected && <div className="absolute top-0 bottom-0 left-0 w-1 bg-blue-500" />}
