...
const PRIORITY_STYLES = {
  ...
  info: {
    ...
    icon: <Info className="h-3.5 w-3.5 text-blue-500 flex-shrink-0" />,
  },
  suggestion: {
  ...
  return createPortal(
    <>
          ...
          >
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-indigo-200 text-[11px] capitalize">{role} assistant</p>
              </div>
              {shadowMode && (
            ...
            {chips.length > 0 && (
              <div className="flex flex-wrap gap-1 px-3 py-2 border-b border-slate-100 bg-slate-50 flex-shrink-0">
                {chips.map((chip) => (
                  <span key={chip} className="text-[10px] font-medium bg-indigo-50 text-indigo-700 px-2 py-0.5 rounded-full border border-indigo-100">
                    {chip}
                  </span>
                ))}
              </div>
            ...
            <div className="px-3 py-2.5 border-t border-slate-100 bg-slate-50 flex-shrink-0">
              <button
                onClick={refresh}
                disabled={loading}
                className="w-full flex items-center justify-center gap-2 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold transition-colors disabled:opacity-50"
              >
      ...
      <button
        ...
        style={{ position: 'fixed', bottom: '24px', right: '24px', zIndex: 9999, background: 'linear-gradient(135deg, #4F46E5, #6366F1)' }}
