  ...
  return (
    <div className="rounded-3xl bg-white shadow-[0_1px_4px_rgba(15,23,42,0.06),0_8px_28px_rgba(15,23,42,0.05)] overflow-hidden">
      ...
      <div className="px-5 pb-4">
          ...
          <motion.div key={stage} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl bg-gradient-to-br from-slate-50 to-slate-100 p-5">
            ...
            {isFirst && (
              <div className="flex gap-6 mt-4">
                <div className="flex items-center gap-2"><Users className="h-5 w-5 text-blue-500" /><span className="text-[15px] text-slate-700"><b className="text-slate-900">{aheadOfYou}</b> {mode === 'video' ? 'ahead in line' : 'ahead of you'}</span></div>
        ...
        {isDone && (
          <motion.div key="done" initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} className="rounded-2xl p-5 bg-gradient-to-br from-blue-50 to-indigo-50 text-center">
            ...
            return (
              <div key={s.key} className="flex items-center flex-1 last:flex-none">
                <div className="flex flex-col items-center gap-1">
                  <div className={cn("h-9 w-9 rounded-full flex items-center justify-center transition-colors relative", done || active ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-400")}>
                  ...
                  <span className={cn("text-[10px] font-semibold", active ? "text-blue-600" : done ? "text-slate-600" : "text-slate-400")}>{s.short}</span>
                </div>
                {i < stages.length - 1 && <div className={cn("flex-1 h-0.5 mx-1 -mt-4 rounded", done ? "bg-blue-600" : "bg-slate-200")} />}
              </div>
            )
          })}
        </div>
