...
import {
  ...
  BedDouble, CreditCard, Scissors, Heart,
} from "lucide-react"
...
export function PortalLauncher() {
  ...
  const handleLogin = (role: Role, href: string) => {
    setSelectedRole(role); setLoadingRole(role); setRole(role); router.push(href)
  }
    ...
    <section id="launcher" className="scroll-mt-20 py-20 lg:py-28 bg-[#F4F7FB] border-t border-[#EAECF2]">
      <div className="max-w-7xl mx-auto px-5 lg:px-10">
        <div className="flex items-end justify-between gap-4 flex-wrap mb-8">
          <div>
            <p className="text-[12px] font-bold uppercase tracking-[0.14em] text-[#1E3A8A]">Enter your workspace</p>
            ...
            return (
              <button key={group.id} onClick={() => setActiveTab(group.id)}
                className={cn("flex-shrink-0 inline-flex items-center gap-1.5 h-9 px-3.5 rounded-full text-[13px] font-semibold transition-all cursor-pointer whitespace-nowrap border",
                  active ? "bg-[#1E3A8A] text-white border-[#1E3A8A]" : "bg-white text-[#475467] border-[#EAECF2] hover:border-[#D0D5DD] hover:text-[#101828]")}>
                {group.label}
        ...
        <AnimatePresence mode="wait">
          <motion.div key={activeTab}
            ...
            {activeGroup.roles.map(({ role, label, desc, icon: Icon, href }) => {
              ...
              return (
                <button key={role} onClick={() => handleLogin(role, href)} disabled={!!loadingRole}
                  className={cn("group flex items-start gap-3 p-4 rounded-2xl text-left cursor-pointer w-full bg-white border transition-all duration-200",
                    isSelected ? "border-[#1E3A8A] shadow-[0_0_0_1px_#1E3A8A,0_8px_24px_rgba(30,58,138,0.12)]"
                  ...
                  <ArrowRight className={cn("h-4 w-4 flex-shrink-0 mt-0.5 transition-all duration-200", isSelected ? "text-[#1E3A8A]" : "text-[#CBD2DC] group-hover:text-[#1E3A8A] group-hover:translate-x-0.5")} />
                </button>
              )
            })}
          </motion.div>
