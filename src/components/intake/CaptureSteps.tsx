      ...
      {methods.map(m => {
        ...
        return (
          <button
            ...
            aria-pressed={sel}
            className={cn(
              "w-full flex items-center gap-4 p-4 rounded-[20px] border text-left transition-all active:scale-[0.98] focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500",
          ...
          >
            ...
            <div>
              ...
              <p className={cn("text-[13px]", sel ? "text-blue-100" : "text-slate-400")}>{m.desc}</p>
            </div>
          </button>
        )
      })}
    ...
    <div className="pt-2 space-y-4">
      ...
      {scan === 'idle' && (
        <div className="flex gap-3">
          <button onClick={runScan} className="flex-1 h-12 rounded-[16px] bg-blue-600 text-white font-semibold text-[15px] active:scale-[0.97] transition-all flex items-center justify-center gap-2 shadow-[0_4px_12px_rgba(37,99,235,0.25)] focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500">
    ...
    <div className="pt-2 space-y-4">
      <div className="bg-slate-200/60 p-1 rounded-xl flex gap-1 max-w-[200px] mx-auto">
        {(['en', 'hi'] as const).map(l => (
          <button
            key={l}
            onClick={() => setLang(l)}
            className={cn("flex-1 h-9 rounded-lg text-[13px] font-semibold transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500", lang === l ? "bg-white text-slate-900 shadow-sm" : "text-slate-500")}
          >
            {l === 'en' ? 'English' : 'हिंदी'}
          </button>
        ))}
      ...
      <div className="flex flex-col items-center gap-4 py-6">
        <button
          ...
          className={cn(
            "h-24 w-24 rounded-full flex items-center justify-center transition-all active:scale-95 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2",
            state === 'listening' ? "bg-red-500 animate-pulse shadow-[0_0_0_12px_rgba(239,68,68,0.15)]"
              : state === 'processing' ? "bg-amber-400" : "bg-blue-600 shadow-[0_8px_20px_rgba(37,99,235,0.3)]"
          )}
        >
