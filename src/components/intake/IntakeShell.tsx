      ...
      <div className="relative flex flex-col w-full h-[100dvh] bg-[#F2F2F7] overflow-hidden md:w-[440px] md:h-[880px] md:max-h-[calc(100dvh-2rem)] md:rounded-[40px] md:shadow-2xl md:border md:border-white/50">
        ...
        <div className="flex-shrink-0 px-6 pt-6 pb-3">
          <div className="flex justify-center mb-3">
            <img src="/kailash-Logo.png" alt="Kailash Healthcare" className="h-6 w-auto object-contain" />
          </div>
          <div className="flex items-center justify-between mb-2.5 min-h-[22px]">
            {onBack ? (
              <button onClick={onBack} className="text-blue-600 flex items-center gap-1 text-[15px] font-medium -ml-1 active:opacity-60 transition-opacity rounded focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500">
          ...
          <div className="h-1.5 rounded-full bg-slate-200 overflow-hidden" role="progressbar" aria-valuenow={stepNumber} aria-valuemin={1} aria-valuemax={totalSteps} aria-label={`Step ${stepNumber} of ${totalSteps}`}>
            <motion.div className="h-full rounded-full bg-blue-600" initial={false} animate={{ width: `${pct}%` }} transition={reduce ? { duration: 0 } : { duration: 0.35, ease: [0.16, 1, 0.3, 1] }} />
          </div>
        ...
        <div className="flex-shrink-0 px-6 pt-3 pb-6 bg-gradient-to-t from-[#F2F2F7] via-[#F2F2F7] to-transparent">
          <button
            ...
            className={cn(
              "w-full h-14 rounded-2xl font-semibold text-[17px] flex items-center justify-center gap-2 transition-all active:scale-[0.97] focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2",
              (ctaDisabled || ctaLoading) ? "bg-slate-200 text-slate-400 cursor-not-allowed" : "bg-blue-600 text-white shadow-[0_8px_20px_rgba(37,99,235,0.3)]"
            )}
          >
            {ctaLoading ? (
              <>
