  ...
  return (
    <div className="px-4 py-2.5">
      <div className="flex items-center justify-between mb-0.5">
        ...
        <button onClick={onEdit} className="text-blue-600 text-[12px] font-semibold flex items-center gap-1 active:opacity-60 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 rounded">
...
export function ReviewStep({ form, onEdit }: { form: IntakeForm; onEdit: (id: StepId) => void }) {
  ...
  return (
    <div className="h-full overflow-y-auto pr-1 space-y-2.5">
      <div className="bg-white rounded-[16px] overflow-hidden shadow-[0_1px_3px_rgba(0,0,0,0.04)] divide-y divide-slate-100">
        ...
        <Row label="Consultation" onEdit={() => onEdit('consultType')}>
          <p className="text-[15px] text-slate-900 font-medium flex items-center gap-1.5">
            {isVideo ? <Video className="h-4 w-4 text-blue-600" /> : <Building2 className="h-4 w-4 text-blue-600" />}
        ...
        <Row label="Department" onEdit={() => onEdit('department')}><Chips items={form.departments} accent /></Row>
      </div>
      ...
      {form.dishaConsent && (
        <div className="flex items-center gap-2.5 px-4 py-2.5 bg-blue-50 rounded-[14px]">
          <QrCode className="h-5 w-5 text-blue-600 flex-shrink-0" aria-hidden="true" />
          <p className="text-[13px] text-blue-800 font-medium truncate">Family link will be created{form.familyPhone ? ` · ${form.familyPhone}` : ''}</p>
        </div>
      )}
    ...
    <div className="h-[100dvh] w-full overflow-hidden bg-slate-100 flex md:items-center md:justify-center">
      <div className="relative flex flex-col w-full h-[100dvh] bg-[#F2F2F7] overflow-hidden md:w-[440px] md:h-[880px] md:max-h-[calc(100dvh-2rem)] md:rounded-[40px] md:shadow-2xl md:border md:border-white/50">
        <div className="flex-1 min-h-0 overflow-hidden px-6 flex flex-col items-center justify-center text-center">
            ...
            <>
              ...
              <div className="w-full bg-white rounded-[20px] shadow-[0_4px_20px_rgba(0,0,0,0.05)] p-4 mb-5 text-left">
                <div className="flex items-center gap-3">
                  <span className="h-11 w-11 rounded-2xl bg-blue-50 flex items-center justify-center flex-shrink-0"><Video className="h-5.5 w-5.5 text-blue-600" /></span>
                  <div>
          ...
          {familyToken && (
            <div className="w-full bg-white rounded-[20px] p-4 shadow-[0_4px_16px_rgba(0,0,0,0.05)] mb-5 flex items-center gap-4">
              ...
              <div className="text-left flex-1 min-w-0">
                <p className="text-[13px] font-bold text-slate-900 flex items-center gap-1.5"><QrCode className="h-4 w-4 text-blue-600" /> Family tracking</p>
                ...
                <button onClick={() => { const url = `${window.location.origin}/family-track/${familyToken}`; if (navigator.share) navigator.share({ title: 'Patient Status', url }); else navigator.clipboard.writeText(url) }}
                  className="inline-flex items-center gap-1.5 text-[12px] font-semibold text-blue-600 active:opacity-60">
          ...
          <button onClick={() => router.push('/patient/dashboard')} className="w-full h-14 rounded-2xl font-semibold text-[17px] text-white bg-blue-600 hover:bg-blue-700 transition-all shadow-[0_4px_14px_rgba(37,99,235,0.3)] active:scale-[0.97] focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500">
            Go to My Dashboard
          </button>
        </div>
      </div>
