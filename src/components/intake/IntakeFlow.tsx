    ...
    return (
      <div className="min-h-[100dvh] bg-[#F2F2F7] flex flex-col md:bg-slate-100 md:py-12">
        <div className="flex-1 w-full max-w-[420px] mx-auto flex flex-col items-center justify-center text-center bg-[#F2F2F7] md:rounded-[40px] md:shadow-2xl overflow-hidden border-x border-slate-200/50 md:border md:border-white/50 px-8">
          <img src="/kailash-Logo.png" alt="Kailash Healthcare" className="h-12 w-auto object-contain mb-8" />
          ...
          <button
            onClick={goNext}
            className="w-full max-w-[320px] h-14 rounded-2xl font-semibold text-[17px] text-white bg-blue-600 hover:bg-blue-700 transition-all shadow-[0_8px_20px_rgba(37,99,235,0.3)] active:scale-[0.97] flex items-center justify-center gap-2 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2"
          >
  ...
  const renderBody = () => {
    switch (current) {
      ...
      case 'symptoms': {
        ...
        return <ChoiceStep fill columns={2} compact options={SYMPTOMS.map(s => ({ value: s, label: s }))} value={form.symptoms} onChange={v => update({ symptoms: v, departments: [] })} multi otherEnabled otherPlaceholder="Describe your problem…" footer={aiBar} />
      }
      case 'department': {
