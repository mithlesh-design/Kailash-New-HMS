      ...
      {TYPES.map(t => {
        ...
        return (
          <button key={t.value} onClick={() => update({ consultationType: t.value })} aria-pressed={sel}
            className={cn("w-full flex items-center gap-4 p-4 rounded-[20px] border text-left transition-all active:scale-[0.98] focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500",
              sel ? "bg-blue-600 border-blue-600 shadow-[0_8px_20px_rgba(37,99,235,0.25)]" : "bg-white border-slate-200")}>
            <div className={cn("h-12 w-12 rounded-2xl flex items-center justify-center flex-shrink-0", sel ? "bg-white/15" : "bg-blue-50")}>
              <Icon className={cn("h-6 w-6", sel ? "text-white" : "text-blue-600")} aria-hidden="true" />
            </div>
            <div>
              <p className={cn("text-[17px] font-semibold", sel ? "text-white" : "text-slate-900")}>{t.label}</p>
              <p className={cn("text-[13px]", sel ? "text-blue-100" : "text-slate-400")}>{t.desc}</p>
            </div>
          </button>
        )
      })}
          ...
          {DOCTORS.map(d => {
            ...
            return (
              <button key={d.id} onClick={() => update({ slotDoctor: d.name })} aria-pressed={sel}
                className={cn("w-full flex items-center gap-3 px-3.5 py-3 rounded-2xl border text-left transition-all active:scale-[0.98] focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500",
                  sel ? "bg-blue-50 border-blue-400 ring-1 ring-blue-300" : "bg-white border-slate-200")}>
                <span className="h-10 w-10 rounded-full bg-blue-100 flex items-center justify-center flex-shrink-0"><Stethoscope className="h-5 w-5 text-blue-600" /></span>
                ...
                {sel && <CheckCircle className="h-5 w-5 text-blue-600 flex-shrink-0" />}
              </button>
            )
          })}
        </div>
              ...
              {days.map(d => (
                <button key={d.value} onClick={() => update({ slotDate: d.value })} aria-pressed={form.slotDate === d.value}
                  className={cn("px-4 py-2 rounded-xl text-[14px] font-medium border transition-all active:scale-95",
                    form.slotDate === d.value ? "bg-blue-600 border-blue-600 text-white" : "bg-white border-slate-200 text-slate-700")}>{d.label}</button>
              ))}
            </div>
          </div>
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {SLOT_TIMES.map(t => (
                <button key={t} onClick={() => update({ slotTime: t })} aria-pressed={form.slotTime === t}
                  className={cn("px-4 py-2 rounded-xl text-[14px] font-medium border transition-all active:scale-95",
                    form.slotTime === t ? "bg-blue-600 border-blue-600 text-white" : "bg-white border-slate-200 text-slate-700")}>{t}</button>
              ))}
            </div>
          </div>
        </>
...
export function PaymentStep({ form, update }: { form: IntakeForm; update: Update }) {
  ...
  return (
    <div className="h-full overflow-y-auto pr-1 pt-1 space-y-4">
      {/* Fee card */}
      <div className="rounded-2xl bg-gradient-to-br from-slate-50 to-blue-50 border border-slate-200 p-4 flex items-center justify-between">
        <div>
          ...
          {([['self', 'Self-pay', Wallet], ['cashless', 'Cashless', ShieldCheck]] as const).map(([val, label, Icon]) => {
            ...
            return (
              <button key={val} onClick={() => update({ payer: val })} aria-pressed={sel}
                className={cn("flex flex-col items-center gap-1.5 py-3.5 rounded-2xl border transition-all active:scale-[0.97] focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500",
                  sel ? "bg-blue-600 border-blue-600 text-white" : "bg-white border-slate-200 text-slate-700")}>
                <Icon className={cn("h-5 w-5", sel ? "text-white" : "text-blue-600")} />
        ...
        <div className="space-y-3">
          ...
          <div className={fieldCard}>
            ...
            {form.name && form.policyHolder !== form.name && (
              <button onClick={() => update({ policyHolder: form.name, insuranceVerified: false })} className="text-[11px] font-semibold text-blue-600 whitespace-nowrap flex-shrink-0">Same as me</button>
            )}
          </div>
