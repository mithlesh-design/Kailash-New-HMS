...
export function RoundsTab({ ip }: { ip: Inpatient }) {
  ...
  return (
    <div>
      <button onClick={() => setRound(true)} className="mb-4 h-10 px-4 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-[13px] font-bold flex items-center gap-1.5"><Stethoscope className="h-4 w-4" /> Start round</button>
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2">
        <button onClick={() => setModal('add_med')} className="h-9 px-3.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-[12.5px] font-bold flex items-center gap-1.5"><Plus className="h-3.5 w-3.5" /> Add medication</button>
...
export function OrdersTab({ ip }: { ip: Inpatient }) {
  ...
  return (
    <div className="space-y-4">
      <button onClick={() => setModal('order_test')} className="h-9 px-3.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-[12.5px] font-bold flex items-center gap-1.5"><Plus className="h-3.5 w-3.5" /> Order test</button>
      <div className="space-y-2">
        {ip.tests.map(t => (
          <div key={t.id} className={cn("rounded-xl p-3", t.critical ? "bg-red-50 border border-red-200" : "bg-slate-50")}>
            ...
            {t.status === 'Ready' && (
              <button onClick={() => acknowledgeTest(ip.patientId, t.id)} className="mt-2 text-[11.5px] font-bold text-blue-600 flex items-center gap-1"><CheckCircle2 className="h-3.5 w-3.5" /> Acknowledge / sign-off</button>
            )}
          </div>
        ))}
...
export function ReferralsTab({ ip }: { ip: Inpatient }) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2">
        <button onClick={() => setModal('refer')} className="h-9 px-3.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-[12.5px] font-bold flex items-center gap-1.5"><GitBranch className="h-3.5 w-3.5" /> Refer</button>
